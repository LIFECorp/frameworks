    ...
    private void filterMediaKeyEvent(KeyEvent keyEvent, boolean needWakeLock) {
        ...
        synchronized(mRingingLock) {
            synchronized(mRCStack) {
                ...
                boolean isHeadsetHookKey = (KeyEvent.KEYCODE_HEADSETHOOK == keyEvent.getKeyCode());
                boolean sipPhone = (isHeadsetHookKey && mode == AudioSystem.MODE_IN_COMMUNICATION);
                if ((mMediaReceiverForCalls != null)
                        && (mIsRinging || (mode == AudioSystem.MODE_IN_CALL)
                                || (mode == AudioSystem.MODE_IN_CALL_2)
                                || (mode == AudioSystem.MODE_IN_CALL_EXTERNAL) || sipPhone)) {
