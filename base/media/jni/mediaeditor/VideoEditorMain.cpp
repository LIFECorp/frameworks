...
#ifndef ANDROID_DEFAULT_CODE
            /* ALPS01426568
               If H264 export format is not supported, callback to MediaArtisNativeHelper.java to
               change the default export format to MPEG4.
             */
            MediaProfiles *sProfiles = MediaProfiles::getInstance();
            int supported = 0;
            int codec = VIDEO_ENCODER_H264;

            supported = sProfiles->getVideoEditorExportParamByName("videoeditor.export.format.supported", codec);
            ALOGD("videoEditor_init: codec %d, supported %d", codec, supported);
            // Check the values retrieved
            if (supported != -1 && pContext->onProgressUpdateMethodId != NULL) {
                pEnv->CallVoidMethod(
                    pContext->engine,
                    pContext->onProgressUpdateMethodId,
                    0, // taskId = 0, Query if H264 export is supprted.
                    supported);
            }
#endif
        }
