...
/// M: parallel layout and hwui.init for improving app first launch time. @{
...
public final class ViewRootImpl implements ViewParent,
    ...
    private static final boolean DEBUG_DISABLE_MULTI_THREAD = SystemProperties.getBoolean(
            "debug.viewroot.disableMT", false);
    ...
    /// M: parallel layout and hwui.init for improving app first launch time. @{
    private final ReentrantLock mParallelProcessLock = new ReentrantLock();
    private boolean mDidParallelProcess = true;
    private Handler mParallelHandler;
    private HandlerThread mParallelThread;
    protected boolean mApplyParallel = false;
    /// @}
    ...
    public ViewRootImpl(Context context, Display display) {
        ...
        /// M: parallel layout and hwui.init for improving app first launch time. @{
        if (DEBUG_DISABLE_MULTI_THREAD) {
            Xlog.v(TAG, "Disable multi thread, this = " + this);
        } else {
            StringBuilder out = new StringBuilder(128);
            out.append(context.getPackageName()).append('.').append(context.getClass().getSimpleName());
            final String[] applyApps = context.getResources().getStringArray(com.mediatek.internal.R.array.viewroot_multi_thread);
            for (String apply : applyApps) {
                if (out.toString().compareTo(apply) == 0) {
                    mApplyParallel = ((context.getApplicationInfo().flags & context.getApplicationInfo().FLAG_SYSTEM)) != 0;
                    break;
                }
            }
            Xlog.v(TAG, "mApplyParallel = " + mApplyParallel + ", this = " + this);
            mDidParallelProcess = mApplyParallel ? false : true;
            if (mApplyParallel) {
                mParallelThread = new HandlerThread("ParallelThread");
                mParallelThread.start();
                mParallelHandler = new Handler(mParallelThread.getLooper());
            }
        }
        /// @}
    }
    ...
    void scheduleTraversals() {
        if (!mTraversalScheduled) {
            ...
            if (mFirstMoveEvent && MOVE_RESPONSE_ENHANCE) {
            ...
            /// M : To improve respose time, 1st move do not wait vsync to do performtraversal
            if (!mFirstMoveEvent || !MOVE_RESPONSE_ENHANCE) {
                //Xlog.v(TAG, "scheduleTraversals: by Vsync ,this = " + this);
    ...
    /// M: parallel layout and hwui.init for improving app first launch time. @{
    private class parallelProcessLayout implements Runnable {
        View mHostView;
        WindowManager.LayoutParams mLp;
        int mDesiredWindowWidth;
        int mDesiredWindowHeight;
        
        public parallelProcessLayout(View hostView, WindowManager.LayoutParams lp,
                int desiredWindowWidth, int desiredWindowHeight) {
            mHostView = hostView;
            mLp = lp;
            mDesiredWindowWidth = desiredWindowWidth;
            mDesiredWindowHeight = desiredWindowHeight;
        }

        public void run() {
            mParallelProcessLock.lock();
            Xlog.v(TAG, "parallelProcessLayout, run()+ , this = " + this);
            try {
                performLayout(mLp, mDesiredWindowWidth, mDesiredWindowHeight);

                // By this point all views have been sized and positioned
                // We can compute the transparent area
                if ((mHostView.mPrivateFlags & View.PFLAG_REQUEST_TRANSPARENT_REGIONS) != 0) {
                    // start out transparent
                    // TODO: AVOID THAT CALL BY CACHING THE RESULT?
                    mHostView.getLocationInWindow(mTmpLocation);
                    mTransparentRegion.set(mTmpLocation[0], mTmpLocation[1],
                            mTmpLocation[0] + mHostView.mRight - mHostView.mLeft,
                            mTmpLocation[1] + mHostView.mBottom - mHostView.mTop);

                    if (DEBUG_DRAW) {
                        Xlog.v(TAG, "ViewRoot gatherTransparentRegion+ : mTransparentRegion = "
                                + mTransparentRegion + ", mTmpLocation = " + mTmpLocation
                                + ",host = " + mHostView + ", this = " + this);
                    }

                    mHostView.gatherTransparentRegion(mTransparentRegion);
                    if (mTranslator != null) {
                ...
                mParallelProcessLock.unlock();
                mDidParallelProcess = true;
                Xlog.v(TAG, "parallelProcessLayout, run()- , this = " + this);
            }
        }
    }
    ...
    private void performTraversals() {
        ...
        /// M: parallel layout and hwui.init for improving app first launch time. @{
        boolean doParallelProcess = false;
        /// @}
        if (mFirst || windowShouldResize || insetsChanged ||
            ...
            try {
                ...
                /// M: parallel layout and hwui.init for improving app first launch time. @{
                if (mApplyParallel) {
                    /// whether to applyParallel
                    int cores = Runtime.getRuntime().availableProcessors();            
                    doParallelProcess = mFirst && !mDidParallelProcess && cores >= 2;
                    if (doParallelProcess) {
                        Xlog.v(TAG, "Parallel process layout");
                        r = new parallelProcessLayout(host, lp, desiredWindowWidth, desiredWindowHeight);
                        mParallelHandler.post(r);
                    }
                }
                /// @}
                if (!hadSurface) {
        ...
        /// M: parallel layout and hwui.init for improving app first launch time.
        if (didLayout && !doParallelProcess) {
        ...
        if (mApplyParallel) {
            mParallelProcessLock.lock();
            mParallelProcessLock.unlock();
            mApplyParallel = false;
            /// Safely to close the thread
            //Thread moribund = mParallelThread;
            mParallelHandler.removeCallbacks(r);
            mParallelThread.quit();
        }
        /// @}
    ...
    void checkThread() {
        /// M: parallel layout and hwui.init for improving app first launch time. @{
        if (mDidParallelProcess && mThread != Thread.currentThread()) {
