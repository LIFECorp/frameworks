...
public class MobileDataStateTracker extends BaseNetworkStateTracker {//[KK] big modify
    ...
    private static class MobileDataStateReceiver extends BroadcastReceiver {
        ...
        private void broadcastDataConnectionStateChanged(Context context, Intent intent, boolean bDataStateChanged) {
            intent.setAction(TelephonyIntents.ACTION_ANY_DATA_CONNECTION_STATE_CHANGED);

            if (bDataStateChanged) {
        ...
        public void onReceive(Context context, Intent intent) {
            if (intent.getAction().equals(TelephonyIntents.
                ...
                if (tracker == null) {
                    Slog.e(TAG, "tracker(null), apn type: " + apnType);
                    return;
                }
                ...
                MobileDataStateTracker tracker = mTrackerMap.get(apnType);
                if (tracker == null) {
                    broadcastDataConnectionStateChanged(context, intent, true);
                    Slog.e(TAG, "tracker(null), apn type: " + apnType);
                    return;
                }
                ...
                if (!isDualTalkMode()  &&                
                    ...
                    if (slot != curSlot) {
                        ...
                        // M: Broadcast original data state change.
                        broadcastDataConnectionStateChanged(context, intent, false); //need to check here using false or true later
                        return;
                    }
                }
               ...
               tracker.mNetworkInfo.setIsAvailable(!intent.getBooleanExtra(PhoneConstants.NETWORK_UNAVAILABLE_KEY, false));
                ...
                broadcastDataConnectionStateChanged(context, intent, dataStateChanged);
