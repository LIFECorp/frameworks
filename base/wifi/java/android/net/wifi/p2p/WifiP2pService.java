...
public class WifiP2pService extends IWifiP2pManager.Stub {
    ...
    public static final int PEER_CONNECTION_USER_ACCEPT     =   BASE + 2;
    /* User rejected a peer request */
    public static final int PEER_CONNECTION_USER_REJECT     =   BASE + 3;
    ...
    /*M: ALPS01303168: wfd sink, for getPeerIpAddress()*/
    ...
    private PendingIntent mClearListenFlagIntent;
    private AlarmManager mAlarmManager;
    private boolean mWaitingForHrToken = false;
    private boolean mListening = false;
    private boolean mReceivedHR = false;
    private boolean mReadHandover = false;
    ...
    private static final String ACTION_CLEAR_LISTEN_FLAG = "com.android.server.WifiManager.action.CLEAR_LISTEN_FLAG";
    ...
    private boolean mTurnOffWifi_NfcP2p = false;
    /** @} */
    ///@}
    ...
    private class P2pStateMachine extends StateMachine {
        ...
        P2pStateMachine(String name, boolean p2pSupported) {
            ...
            if (FeatureOption.MTK_WIFIWPSP2P_NFC_SUPPORT) {
                ...
                mClearListenFlagIntent = PendingIntent.getBroadcast(mContext, CLEAR_LISTEN_FLAG_REQUEST, new Intent(ACTION_CLEAR_LISTEN_FLAG), 0);
                IntentFilter filter = new IntentFilter();
                filter.addAction(WifiP2pService.MTK_NFC_WFA_P2P_HR_RECEIVE_ACTION);
                filter.addAction(WifiP2pService.MTK_NFC_WFA_P2P_HS_RECEIVE_ACTION);
                filter.addAction(WifiP2pService.MTK_NFC_WFA_TAG_RECEIVE_ACTION);
                mContext.registerReceiver(
                    new BroadcastReceiver() {
                        ...
                        public void onReceive(Context context, Intent intent) {
                            ...
                            if (action.equals(WifiP2pService.MTK_NFC_WFA_P2P_HR_RECEIVE_ACTION)) {
                                byte[] token = intent.getByteArrayExtra(WifiP2pService.MTK_NFC_WFA_P2P_EXTRA_HR_P2P_DEV_INFO);
                                if (token != null) {
                                    mReceivedHR = true;
                                    sendMessage(obtainMessage(M_CMD_NFC_HANDOVER_READ, bytesToHexString(token)));
                                } else {
                                    loge("No HR token!");
                                }
                            } else if (action.equals(WifiP2pService.MTK_NFC_WFA_P2P_HS_RECEIVE_ACTION)) {
                                byte[] token = intent.getByteArrayExtra(WifiP2pService.MTK_NFC_WFA_P2P_EXTRA_HS_P2P_DEV_INFO);
                                if (token != null) {
                                    sendMessage(obtainMessage(M_CMD_NFC_HANDOVER_READ, bytesToHexString(token)));
                                } else {
                                    loge("No HS token!");
                                }
                            } else if (action.equals(WifiP2pService.MTK_NFC_WFA_TAG_RECEIVE_ACTION)) {
                                byte[] token = intent.getByteArrayExtra(WifiP2pService.MTK_NFC_WFA_TAG_EXTRA_DEV_INFO);
                                if (token != null) {
                                ...
                                mTmpGoAddress = null;
                            } else if (action.equals(ACTION_CLEAR_LISTEN_FLAG)) {
                                mListening = false;
                            }
                        }
                    },
            ...
            switch (message.what) {
                ...
                case WifiP2pManager.GET_NFC_WPS_CONFIG_TOKEN:
                    replyToMessage(message, WifiP2pManager.GET_NFC_WPS_CONFIG_TOKEN_FAILED,
                            WifiP2pManager.BUSY);
                    break;
                case WifiMonitor.P2P_NFC_DEVICE_FOUND_EVENT:
                    break;
            ...
            switch (message.what) {
               ...
               /** M: NFC Float II @{ */
                case WifiP2pManager.GET_NFC_REQUEST_TOKEN:      //OOB Device Discovery
                    ...
                    String token = mWifiNative.getP2pHandoverRequestToken();
                ...
                case WifiP2pManager.GET_NFC_SELECT_TOKEN:       //Write P2P Tag
                    ...
                    token = mWifiNative.getStaticHandoverSelectToken();
                    ...
                    if (!TextUtils.isEmpty(token) && !token.equals(UNKNOWN_COMMAND)) {
                        ...
                        mWifiNative.p2pListen(120);
                        mListening = true;
                        mAlarmManager.set(AlarmManager.RTC_WAKEUP, System.currentTimeMillis() + CLEAR_OPERATION_MS, mClearListenFlagIntent);
                    } else {
                        logd("Failed to get nfc select token!");
                        replyToMessage(message, WifiP2pManager.GET_NFC_SELECT_TOKEN_FAILED);
                    }
                ...
                case M_CMD_NFC_TAG_READ:
                    ...
                    mWifiNative.p2pNfcTagRead((String)message.obj);
                    mReadHandover = false;
                    break;
                case M_CMD_NFC_HANDOVER_READ:
                    ...
                    mWifiNative.p2pNfcHandoverRead((String)message.obj);
                    mReadHandover = true;
                    break;
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                ...
                case WifiP2pManager.CREATE_GROUP:
                    ...
                    boolean ret = false;
                    ///M: no persistent group at wfd sink mode  @{
                    boolean wfdSink = false;
                    ...
                    if (WifiP2pGroup.PERSISTENT_NET_ID==netId &&
                        false==wfdSink) {
                ...
                case WifiMonitor.P2P_NFC_DEVICE_FOUND_EVENT:
                    ...
                    WifiP2pDevice device = (WifiP2pDevice) message.obj;
                    logd(getName() + "P2P_NFC_DEVICE_FOUND_EVENT, device:" + device
                         + ", mListening:" + mListening + ", mReceivedHR:" + mReceivedHR);
                    if (mThisDevice.deviceAddress.equals(device.deviceAddress)) break;
                    ...
                    if (!mListening) {
                        switch(device.oobRoleIndication) {
                            case WifiP2pDevice.ROLE_DEVICE:
                                if (mWifiNative.p2pNfcConnectWithOob(device.deviceAddress, false)) {
                                    transitionTo(mGroupNegotiationState);
                                }
                                break;
                            case WifiP2pDevice.ROLE_GC:
                                if (mPeers.containsPeer(device.goIface)) {
                                    logd("GO is in device list!");
                                    WifiP2pConfig goConfig = new WifiP2pConfig();
                                    goConfig.deviceAddress = device.goIface;
                                    goConfig.wps.setup = WpsInfo.PBC;
                                    sendMessage(WifiP2pManager.CONNECT, goConfig);
                                } else {
                                    logd("GO is not in device list! Try to find!");
                                    mTmpGoAddress = device.goIface;
                                    mAlarmManager.set(AlarmManager.RTC_WAKEUP, System.currentTimeMillis() + CLEAR_OPERATION_MS, mClearTmpGoIntent);
                                    sendMessage(WifiP2pManager.DISCOVER_PEERS);
                                }
                                break;
                            case WifiP2pDevice.ROLE_GO:
                                if (mWifiNative.p2pNfcConnectWithOob(device.deviceAddress, true)) {
                                    transitionTo(mGroupNegotiationState);
                                }
                                break;
                            default:
                        ...
                        mListening = false;
                        mAlarmManager.cancel(mClearListenFlagIntent);
                        transitionTo(mGroupNegotiationState);
                    }
                    break;
                ...
                case WifiMonitor.P2P_GROUP_STARTED_EVENT:
                    ...
                    if (mGroup.isGroupOwner()) {
                        ...
                        if (mGcFastConnectEnaled) {
                            ...
                            if (FeatureOption.MTK_WIFIWPSP2P_NFC_SUPPORT && mTmpGcIp != null && mTmpGoIp != null) {
                                try {
                                    ...
                                    loge("Error configuring interface " + mGroup.getInterface() + ", :" + e);
                                }
                            } else {
                            /** @} */
                                mWifiNative.setP2pGroupIdle(mGroup.getInterface(), GROUP_IDLE_TIME_S);
    ...
    class GroupCreatedState extends State {
        ...
        public void enter() {
            ...
            if (mAutonomousGroup) {
                ///M: label P2pConnectionChangedBroadcast from UserAuthorizingJoinState  @{
                LogRec stateMachineLogRec = mP2pStateMachine.getLogRec(mP2pStateMachine.getLogRecCount()-1);
                if ( null!=stateMachineLogRec &&
                     mUserAuthorizingJoinState==stateMachineLogRec.getOriginalState() &&
                    (PEER_CONNECTION_USER_ACCEPT==stateMachineLogRec.getWhat()||
                     PEER_CONNECTION_USER_REJECT==stateMachineLogRec.getWhat()) ) {
                    if (DBG) logd("[GO] GC connection coming case, state transit - enter");
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                ...
                case WifiMonitor.P2P_NFC_DEVICE_FOUND_EVENT:
                    ...
                    device = (WifiP2pDevice) message.obj;
                    logd(getName() + "P2P_NFC_DEVICE_FOUND_EVENT, device:" + device + ", mReadHandover:" + mReadHandover);
                    if (mThisDevice.deviceAddress.equals(device.deviceAddress)) break;
                    mPeers.update(device);
                    sendPeersChangedBroadcast();
                    if (mGroup.isGroupOwner() && device.oobRoleIndication == WifiP2pDevice.ROLE_DEVICE && !mReadHandover) {
                        mWifiNative.p2pNfcInvite(device.deviceAddress);
                    }
                    break;
                case WifiP2pManager.GET_NFC_CONFIG_TOKEN:
                    if (!FeatureOption.MTK_WIFIWPSP2P_NFC_SUPPORT) {
        ...
        public void exit() {
            ...
            LogRec stateMachineLogRec = mP2pStateMachine.getLogRec(mP2pStateMachine.getLogRecCount()-1);
            if ( null!=stateMachineLogRec &&
                 mGroupCreatedState==stateMachineLogRec.getDestState() &&
                (PEER_CONNECTION_USER_ACCEPT==stateMachineLogRec.getWhat()||
                 PEER_CONNECTION_USER_REJECT==stateMachineLogRec.getWhat()) ) {
                if (DBG) logd("[GO] GC connection coming case, state transit - exit");
                sendP2pConnectionChangedBroadcast(P2pStatus.MTK_EXPAND_01);
            } else 
            ///@}
            /*M: ALPS00677009: broadcast the group removed reason*/
    ...
    class UserAuthorizingJoinState extends State {
        ...
        public void enter() {
            if (DBG) logd(getName());
            ///M: dialog handling move to wfd framework when this device is wfd sink  @{
            if ( null!=mThisDevice.wfdInfo &&
                 ((WifiP2pWfdInfo.PRIMARY_SINK==mThisDevice.wfdInfo.getDeviceType()) ||
                  (WifiP2pWfdInfo.SOURCE_OR_PRIMARY_SINK==mThisDevice.wfdInfo.getDeviceType())) ) {
    ...
    private void handleGroupRemoved() {
        ...
        if (mGroup.isGroupOwner()) {
            ...
            if (mDhcpStateMachine != null) {
                ...
                logd("mDhcpStateMachine is null!");
            }
            ///@}
        }
    ...
    private void sendP2pConnectionChangedBroadcast(P2pStatus reason) {
        ...
        if (P2pStatus.NO_COMMON_CHANNEL == reason) {
            ...
            if (DBG) logd("[GO] broadcast with reason=-2");
    ...
    public String getPeerIpAddress(String deviceAddress) {
        ...
        try {
            ...
            logd("getClientIp before read file line");
            ...
            while((str != null) && (str.length() != 0)) {
                ...
                if (str.indexOf(deviceAddress) != -1) {
    ...
    private void sendNfcRequestTokenBroadcast(String token) {
        Intent intent = new Intent(WifiP2pService.MTK_NFC_WFA_P2P_HR_ACTION);
        intent.addFlags(Intent.FLAG_RECEIVER_REGISTERED_ONLY_BEFORE_BOOT);
        intent.putExtra(WifiP2pService.MTK_NFC_WFA_P2P_EXTRA_HR_P2P_DEV_INFO, hexStringToBytes(token));
        mContext.sendStickyBroadcastAsUser(intent, UserHandle.ALL);
    }
    
    private void sendNfcSelectTokenBroadcast(String token) {
        Intent intent = new Intent(WifiP2pService.MTK_NFC_WFA_TAG_WRITE_ACTION);
        intent.addFlags(Intent.FLAG_RECEIVER_REGISTERED_ONLY_BEFORE_BOOT);
        intent.putExtra(WifiP2pService.MTK_NFC_WFA_TAG_EXTRA_DEV_INFO, hexStringToBytes(token));
        mContext.sendStickyBroadcastAsUser(intent, UserHandle.ALL);
    }

    private void sendNfcConfigTokenBroadcast(String token) {
        Intent intent = new Intent(WifiP2pService.MTK_NFC_WFA_CFG_TAG_WRITE_ACTION);
        intent.addFlags(Intent.FLAG_RECEIVER_REGISTERED_ONLY_BEFORE_BOOT);
        intent.putExtra(WifiP2pService.MTK_NFC_WFA_TAG_EXTRA_DEV_INFO, hexStringToBytes(token));
        mContext.sendStickyBroadcastAsUser(intent, UserHandle.ALL);
    }
    ...
    private void sendNfcWpsConfigTokenBroadcast(String token) {
        Intent intent = new Intent(WifiP2pService.MTK_WPS_NFC_TESTBED_P2P_AUTOGO_AS_SEL_ACTION);
        intent.addFlags(Intent.FLAG_RECEIVER_REGISTERED_ONLY_BEFORE_BOOT);
        intent.putExtra(WifiP2pService.MTK_WPS_NFC_TESTBED_EXTRA_PASSWORD, hexStringToBytes(token));
        mContext.sendStickyBroadcastAsUser(intent, UserHandle.ALL);
    }
    ...
    public byte[] getP2pHandoverSelectToken() {
        ...
        String token = mP2pStateMachine.mWifiNative.getP2pHandoverSelectToken();
