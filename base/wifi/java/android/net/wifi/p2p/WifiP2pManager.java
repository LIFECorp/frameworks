...
public class WifiP2pManager {
   ...
   /**
     ...
     * @hide
     */
    public void setGCInviteResult(Channel c, boolean accept, ActionListener listener) {
        checkChannel(c);
        if (true == accept) {
            c.mAsyncChannel.sendMessage(WifiP2pService.PEER_CONNECTION_USER_ACCEPT, 0, c.putListener(listener));
        } else {
            c.mAsyncChannel.sendMessage(WifiP2pService.PEER_CONNECTION_USER_REJECT, 0, c.putListener(listener));
        }
    }

    /** M: NFC Float II @{ */
    /**
     * Get NFC request token
     *
     * @param c is the channel created at {@link #initialize}
     * @param listener for callbacks on success or failure. Can be null.
     * @hide
     */
    public void generateNfcRequestToken(Channel c, ActionListener listener) {
        checkChannel(c);
        c.mAsyncChannel.sendMessage(GET_NFC_REQUEST_TOKEN, 0, c.putListener(listener));
    }
    
    /**
     * Get NFC select token
     *
     * @param c is the channel created at {@link #initialize}
     * @param listener for callbacks on success or failure. Can be null.
     * @hide
     */
    public void generateNfcSelectToken(Channel c, ActionListener listener) {
        checkChannel(c);
        c.mAsyncChannel.sendMessage(GET_NFC_SELECT_TOKEN, 0, c.putListener(listener));
    }

    /**
     * Get NFC config token
     *
     * @param c is the channel created at {@link #initialize}
     * @param listener for callbacks on success or failure. Can be null.
     * @hide
     */
    public void generateNfcConfigurationToken(Channel c, ActionListener listener) {
        checkChannel(c);
        c.mAsyncChannel.sendMessage(GET_NFC_CONFIG_TOKEN, 0, c.putListener(listener));
    }

    /**
     * Get NFC WPS config token
     *
     * @param c is the channel created at {@link #initialize}
     * @param listener for callbacks on success or failure. Can be null.
     * @hide
     */
    public void generateNfcWpsConfigurationToken(Channel c, ActionListener listener) {
        checkChannel(c);
        c.mAsyncChannel.sendMessage(GET_NFC_WPS_CONFIG_TOKEN, 0, c.putListener(listener));
    }
