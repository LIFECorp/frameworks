...
public:
    ...
    virtual void output(int level, uint32_t logFlags, OpenGLRenderer* renderer) const {
        if (logFlags & kOpLogFlag_MTK && renderer) {
            OP_LOG("Save flags 0x%x, count %d", mFlags, renderer->getSaveCount() - 1);
        } else {
            OP_LOG("Save flags 0x%x", mFlags);
        }
    }
...
public:
    ...
    virtual void output(int level, uint32_t logFlags, OpenGLRenderer* renderer) const {
        if (logFlags & kOpLogFlag_MTK && renderer) {
            OP_LOG("Restore to count %d", mSaveCount + mCount);
        } else {
...
public:
    ...
    virtual void output(int level, uint32_t logFlags, OpenGLRenderer* renderer) const {
        if (logFlags & kOpLogFlag_MTK && renderer) {
            OP_LOG("SaveLayer%s " RECT_STRING ", alpha %d, mode %d, flags 0x%x, count %d <%p>",
                (isSaveLayerAlpha() ? "Alpha" : ""), RECT_ARGS(mArea), mAlpha, mMode, mFlags, renderer->getSaveCount() - 1, this);
        } else {
            OP_LOG("SaveLayer%s " RECT_STRING ", alpha %d, mode %d, flags 0x%x <%p>",
...
public:
    DrawSomeTextOp(const char* text, int bytesCount, int count, SkPaint* paint)
        ...
        if (g_HWUI_debug_display_list || g_HWUI_debug_font_renderer) {
            int total = mCount;
...
public:
    ...
    virtual status_t multiDraw(OpenGLRenderer& renderer, Rect& dirty,
        ...
        for (unsigned int i = 0; i < ops.size(); i++) {
            ...
            if(g_HWUI_debug_font_renderer) ALOGD("multiDraw \"%s\"", op.mLogText);
