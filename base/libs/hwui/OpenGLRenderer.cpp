...
#include "MTKDebug.h"
#include "LayerRenderer.h" /// M: add to do copyLayer
...
status_t OpenGLRenderer::prepareDirty(float left, float top,
        float right, float bottom, bool opaque) {
    ATRACE_CALL();
    LAYER_RENDERER_LOGD("prepareDirty (%.2f, %.2f, %.2f, %.2f) opaque %d <%p>", left, top, right, bottom, opaque, this);
...
status_t OpenGLRenderer::clear(float left, float top, float right, float bottom, bool opaque) {
    ...
    if (!opaque || mCountOverdraw) {
        LAYER_RENDERER_LOGD("clear (%.2f, %d - %.2f = %.2f, %.2f - %.2f = %.2f, %.2f - %.2f = %.2f) opaque %d <%p>", left, mSnapshot->height, bottom,
...
void OpenGLRenderer::finish() {
    ATRACE_CALL();
    LAYER_RENDERER_LOGD("finish <%p>", this);
    ...
    if (layer->isTextureLayer()) {
        if (g_HWUI_debug_dumpTextureLayer) {
            ...
            if (w !=0 && h != 0) {
                ...
                LayerRenderer::copyLayer(layer, &bitmap);
