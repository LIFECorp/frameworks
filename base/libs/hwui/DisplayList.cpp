...
void DisplayList::outputViewProperties(const int level) {
    ...
    if (mAlpha < 1) {
        if (mCaching) {
            ...
            ALOGD("%*sSaveLayerAlpha "RECT_STRING", alpha %d, mode %d, flags 0x%x by property", level * 2, "",
                    (float) 0, (float) 0, (float) mRight - mLeft, (float) mBottom - mTop,
                    (int)(mAlpha * 255), SkXfermode::kSrcOver_Mode, flags);
        }
    }
    if (clipToBoundsNeeded) {
    ...
    if (mAlpha < 1) {
        if (mCaching) {
            ...
            handler(mSaveLayerOp->reinit(0, 0, mRight - mLeft, mBottom - mTop,
                    ...
                    mClipToBounds);
            DISPLAY_LIST_LOGD("%*sSaveLayerAlpha "RECT_STRING", alpha %d, mode %d, flags 0x%x, count %d <%p> by property",
                    level * 2, "", (float) 0, (float) 0, (float) mRight - mLeft, (float) mBottom - mTop,
                    (int)(mAlpha * 255), SkXfermode::kSrcOver_Mode, saveFlags, renderer.getSaveCount() - 1, mSaveLayerOp);
        }
    }
    if (clipToBoundsNeeded) {
...
void DisplayList::iterate(OpenGLRenderer& renderer, T& handler, const int level) {
    ...
    DISPLAY_LIST_LOGD("%*sSave flags 0x%x, count %d by property", (level + 1) * 2, "",
    ...
    if (mClipToBounds && renderer.quickRejectNoScissor(0, 0, mWidth, mHeight)) {
        ...
        DISPLAY_LIST_LOGD("%*sRestore to count %d (%p, %s) rejected by property", (level + 1) * 2, "", restoreTo, this, mName.string());
    ...
    handler(mRestoreToCountOp->reinit(restoreTo), PROPERTY_SAVECOUNT, mClipToBounds);
    DISPLAY_LIST_LOGD("%*sRestore to count %d by property", (level + 1) * 2, "", restoreTo);
    renderer.restoreToCount(restoreTo);
    renderer.setOverrideLayerAlpha(1.0f);
}
