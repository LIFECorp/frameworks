...
final class WifiDisplayController implements DumpUtils.Dump {
    ...
    private static final int WFD_SINK_IP_RETRY_COUNT = 10;
    ...
    private void updateWfdInfo(boolean enable)
    {
        ...
        if (!enable) {
            ...
            mWifiP2pManager.setWFDInfo(mWifiP2pChannel, wfdInfo, new ActionListener() {
             ...
             ///M:@{ WFD Sink Support
             //Reset as source
             if (FeatureOption.MTK_WFD_SINK_SUPPORT && mSinkEnabled) {                 
                 mSinkEnabled = false;               
           }            
            ///@}   
            ...
            if (FeatureOption.MTK_WFD_SINK_SUPPORT && mSinkEnabled) {
                if (FeatureOption.MTK_WFD_SINK_UIBC_SUPPORT) {
                    ...
                    wfdInfo.setUibcSupported(false);
                }
            } else {
                wfdInfo.setUibcSupported(true);
            }          
                ...
                mWifiP2pManager.setWFDInfo(mWifiP2pChannel, wfdInfo, new ActionListener() {
                    ...
                    public void onSuccess() {
                        ...
                        if (mWfdEnabling) {
                            ...
                            // If WFD sink is supported, don't auto scan when WFD is enabled
            ...
            mConfirmConnectDialog = new AlertDialog.Builder(mContext)
                ...
                .setPositiveButton(mResource.getString(R.string.accept), new OnClickListener() {
                        ...
                        public void onClick(DialogInterface dialog, int which) {
                            ...
                            mWifiP2pManager.setGCInviteResult(mWifiP2pChannel, true, null);                            
                ...
                .setNegativeButton(mResource.getString(R.string.decline), new OnClickListener() {
                        ...
                        public void onClick(DialogInterface dialog, int which) {
                            ...
                            mWifiP2pManager.setGCInviteResult(mWifiP2pChannel, false, null);  
    ...
    public void requestEnableSink(boolean enable) {
        Slog.i(TAG, Thread.currentThread().getStackTrace()[2].getMethodName() + ",enable = " + enable + ",Connected = " + mIsWFDConnected + ", option = " + FeatureOption.MTK_WFD_SINK_SUPPORT + ", WfdEnabled = " + mWfdEnabled);
    ...
    public void requestWaitConnection(Surface surface) {
        ...
        mWifiP2pManager.createGroup(mWifiP2pChannel, new ActionListener() {
            @Override
            public void onSuccess() {                
                Slog.d(TAG, "create group succeeded.");
            ...
            @Override
            public void onFailure(int reason) {                
                Slog.d(TAG, "create group failed.");
    ...
    private synchronized void disconnectWfdSink() {        
        ...
        if (isSinkState(SinkState.SINK_STATE_WAITING_P2P_CONNECTION) ||
            ...
            mWifiP2pManager.removeGroup(mWifiP2pChannel, new ActionListener() {
                ...
                @Override
                public void onFailure(int reason) {
                    Slog.i(TAG, "Failed to disconnect WFD sink (P2P).");
    ...
    private void handleSinkP2PConnection(NetworkInfo networkInfo){
        Slog.i(TAG, "handleSinkP2PConnection()");
                    ...
                    if (group.getOwner().deviceAddress.equals(mThisDevice.deviceAddress)) {
                        ...
                        for (WifiP2pDevice c : group.getClientList()) {
                            ...
                            if (isWifiDisplaySource(c) && mSinkDeviceName.equals(c.deviceName)){
                                ...
                                Slog.i(TAG, "Found ! Sink name:" + mSinkDeviceName + "mac address:" + mSinkMacAddress);
    ...
    private final Runnable mGetSinkIpAddr = new Runnable() {
        ...
        public void run() {                    
            ...
            if (mSinkIpAddress == null) {
                ...
                //Add port number
                mSinkIpAddress += ":" + DEFAULT_CONTROL_PORT;
    ...
    private void updateIfSinkConnected(boolean connected) {
        ...
        // Show toast when connected. LENGTH_SHORT: 2s, LENGTH_LONG: 3.5s
        if (connected) {
            Toast.makeText(
                mContext, 
                com.mediatek.internal.R.string.wifi_display_enjoy_screen,
        ...
        // Make backlight to turn off always when connected
