...
public class CdmaSubscriptionSourceManager extends Handler {
	...
	// for 3g dongle support
        log("3g dongle getDefaultCdmaSubscriptionSource->subscriptionSource1: "+subscriptionSource);
        if (FeatureOption.MTK_EXTERNAL_DONGLE_SUPPORT)
            subscriptionSource = Settings.Global.getInt(mContext.getContentResolver(),
                Settings.Global.CDMA_SUBSCRIPTION_MODE, SUBSCRIPTION_FROM_RUIM);
        log("3g dongle getDefaultCdmaSubscriptionSource->subscriptionSource2: "+subscriptionSource);
