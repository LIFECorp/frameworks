...
import com.android.internal.telephony.PsArbitrator;
...
//public final class GsmServiceStateTracker extends ServiceStateTracker {
    ...
    //for PS arbitrator
    protected static final int EVENT_POLL_STATE_GPRS_2G = 500520;
    protected static final int EVENT_POLL_STATE_GPRS_34G = 500521;

    //MTK-Add-START [ALPS01268462]
    protected static final int EVENT_RADIO_NOT_AVAILABLE = 500522;
    //MTK-Add-END [ALPS01268462]
...
//MTK-END [mtk03851][111124]MTK added
    ...
    //for LteDc project
    private int mPreviousRadioMode = GeminiNetworkSubUtil.MODE_FLIGHT_MODE;
...
//MTK-START [mtk03851][111124]MTK added
        ...
        // LteDcServiceManager extends GsmServiceStateTracker,
        // it should not has no LteDcManager instance
        if(PhoneFactory.isLteDcSupport() && mCi.getLteDcManager() != null) {
            mCi.getLteDcManager().registerForRadioStateChanged(this, EVENT_RADIO_STATE_CHANGED, null);
            //[ALPS01268462] 
            mCi.getLteDcManager().registerForNotAvailable(this, EVENT_RADIO_NOT_AVAILABLE, null);
        }
        ...
        if (!FeatureOption.MTK_GEMINI_SUPPORT) {
            ...
            /* ALPS00447303 */
            loge("Set mDesiredPowerState in GsmServiceStateTracker constructor. simMode="+simMode+",airplaneMode="+airplaneMode);
            ...
            if (PhoneFactory.isLteDcSupport()&&(!(this instanceof LteDcServiceStateTracker))){
                ...
                // For CTA, available to call sos call when no sim inserted
                if (currentPhone != null){
                    if ((currentPhone.getIccCard()) != null){
                        if (!(currentPhone.getIccCard().hasIccCard()) && !(airplaneMode > 0)) {
                            mDesiredPowerState = true;
                        }
                    } else {
                        loge("currentPhone.getIccCard() is null");
                    }
                } else {
                     loge("currentPhone is null");
                }
             }
        }
        ...
        loge("Final mDesiredPowerState for single sim GsmServiceStateTracker constructor. [" + mDesiredPowerState + "] airplaneMode=" + airplaneMode);
    ...
    public void dispose() {
        ...
        // it should not has no LteDcManager instance
        if(PhoneFactory.isLteDcSupport() && mCi.getLteDcManager() != null) {
            mCi.getLteDcManager().unregisterForRadioStateChanged(this);
            //[ALPS01268462]
            mCi.getLteDcManager().unregisterForNotAvailable(this);
        }
    ...
    public void handleMessage (Message msg) {
        ...
        switch (msg.what) {
            ...
            case EVENT_RADIO_NOT_AVAILABLE:
                if(PhoneFactory.isLteDcSupport() && mCi.getLteDcManager() != null) {
                    mPreviousRadioMode = GeminiNetworkSubUtil.MODE_FLIGHT_MODE;
                    log("re-registerForRadioStateChanged and reset mPreviousRadioMode");
                    mCi.getLteDcManager().registerForRadioStateChanged(this, EVENT_RADIO_STATE_CHANGED, null);
                }
            ...
            case EVENT_RADIO_STATE_CHANGED:
                ...
                //MTK-START : For LTE dc
                if (PhoneFactory.isLteDcSupport() && mFirstRadioChange
                        && LteModemSwitchHandler.getActiveModemType() == LteModemSwitchHandler.MD_TYPE_LTNG){
                    setCurrentPreferredNwType();
                    //MTK-Add-START [ALPS01268462]
                    if(mCi.getLteDcManager() != null) {
                        if((mCi.getLteDcManager()).getRadioState().isAvailable()){
                            log("need to unregisterForRadioStateChanged after boot-up flow");
                            mCi.getLteDcManager().unregisterForRadioStateChanged(this);
                        }
                    }
                    //MTK-Add-END [ALPS01268462]
                ...
                if (ar.exception == null) {
                    ...
                    if (states.length >= 3) {
                        try {
                            ...
                            loge("error parsing location: " + ex);
                        }
                    }
            ...
            case EVENT_POLL_STATE_NETWORK_SELECTION_MODE:
                ...
                if (PhoneFactory.isLteDcSupport() && LteModemSwitchHandler.getActiveModemType() == LteModemSwitchHandler.MD_TYPE_LTNG &&
                    msg.what == EVENT_POLL_STATE_GPRS)
                {
                    if (PsArbitrator.PS_MODE_GSM == msg.arg2) {
                        log("handle EVENT_POLL_STATE_GPRS, the event is come from 2G");
                        handlePollStateResult(EVENT_POLL_STATE_GPRS_2G, ar);
                    } else if (PsArbitrator.PS_MODE_UMTS == msg.arg2 || PsArbitrator.PS_MODE_LTE == msg.arg2) {
                        log("handle EVENT_POLL_STATE_GPRS, the event is come from 34G");
                        handlePollStateResult(EVENT_POLL_STATE_GPRS_34G, ar);
                    } else {
                        log("handle EVENT_POLL_STATE_GPRS, the event is come unknown");
            ...
            case EVENT_SIM_RECORDS_LOADED:
                ...
                if(oldImsi == null || !oldImsi.equals(newImsi)) {	  
                    log("GSST: Sim"+ (mSimId+1) + " Card changed  lastImsi is " + oldImsi + " newImsi is " + newImsi); 
                    bImsiChanged = true;
                ...
                if(bImsiChanged && mSS.getIsManualSelection()) {
                    log("GSST: service state is out of service with manual network selection mode,  setNetworkSelectionModeAutomatic " );
            ...
            case EVENT_SET_GPRS_CONN_TYPE_DONE:
                log("GSST EVENT_SET_GPRS_CONN_TYPE_DONE");
                ar = (AsyncResult) msg.obj;
                // M: For single SIM, turn on radio after set connection type done.
                if (!FeatureOption.MTK_GEMINI_SUPPORT && !PhoneFactory.isLteDcSupport()) {
                    setPowerStateToDesired();
                }
                if(ar.exception != null){
            ...
            case EVENT_SET_GPRS_CONN_RETRY:
                log("EVENT_SET_GPRS_CONN_RETRY");
                ServiceState ss = mPhone.getServiceState();
                if (ss == null) break;
                log("GSST EVENT_SET_GPRS_CONN_RETRY ServiceState " + mSS.getVoiceRegState());
                ...
                log("GSST EVENT_SET_GPRS_CONN_RETRY airplanMode " + airplanMode);
                if (airplanMode > 0) {
                    break;
                }
            ...
            case EVENT_DATA_CONNECTION_DETACHED:
                log("EVENT_DATA_CONNECTION_DETACHED: set gprsState=STATE_OUT_OF_SERVICE");
                ...
                //}
                updateDataNetworkType(mSimId, networkTypeToString(ps_networkType));
                //MTK - END : For Lte Dc 
            ...
            case EVENT_SIM_PLUG_OUT: //ALPS00296298
                ...
                if(PhoneFactory.isLteDcSupport() && mCi.getLteDcManager() != null
                        && LteModemSwitchHandler.getActiveModemType() == LteModemSwitchHandler.MD_TYPE_LTNG) {
                    mPreviousRadioMode = GeminiNetworkSubUtil.MODE_FLIGHT_MODE;
                    log("reset mPreviousRadioMode due to EVENT_SIM_PLUG_OUT ");
                }
            ...
            case EVENT_FEMTO_CELL_INFO:
                ...
                break;
            case EVENT_DATA_USAGE_CHANGE:
                if (PhoneFactory.isLteDcSupport() && LteModemSwitchHandler.getActiveModemType() == LteModemSwitchHandler.MD_TYPE_LTNG){
                    //[ALPS01427971] fix set DUAL_SIM_MODE to wrong value in IPO shutdown flow
                    //setPowerStateToDesired();
                    setRadioPowerForDataUsageChange();
                }
                break;
            default:
    ...
    // For MMDSDC structure, needs to decide whether turn on protocol1/protocol2 radio 
    // according to network mode and data usage status
    private int getDesiredRadioMode() {
        if(PhoneFactory.isLteDcSupport() && LteModemSwitchHandler.getActiveModemType() == LteModemSwitchHandler.MD_TYPE_LTNG) {
            int preferredNetworkMode = RILConstants.PREFERRED_NETWORK_MODE;
            int networkMode = Settings.Global.getInt(mPhone.getContext().getContentResolver(),
                    Settings.Global.PREFERRED_NETWORK_MODE, preferredNetworkMode);
            boolean isAnyApnCtxEnabled = mPhone.mDcTracker.isAnyApnCtxEnabled();
            log("networkMode: " + networkMode + " isAnyApnCtxEnabled: " + isAnyApnCtxEnabled);
            ...
            //if(!mPhone.isSimInsert()) {
            if(!currentPhone.getIccCard().hasIccCard()) {
                // For CTA, available to call sos call when no sim inserted
                // Turn on G protocol, and turn off TL protocol in MMDC mode
                return GeminiNetworkSubUtil.MODE_SIM2_ONLY;
            } else {
                // turn off protocol 1 radio if no data use
                if(isAnyApnCtxEnabled) {
                    // decide radio modem depends on network mode
                    switch (networkMode) {
                        case RILConstants.NETWORK_MODE_GSM_ONLY:
                            return GeminiNetworkSubUtil.MODE_SIM2_ONLY;
                        case RILConstants.NETWORK_MODE_WCDMA_ONLY:
                        case RILConstants.NETWORK_MODE_LTE_ONLY:
                            return GeminiNetworkSubUtil.MODE_SIM1_ONLY;
                        default:
                            return GeminiNetworkSubUtil.MODE_DUAL_SIM;
                    }
                } else {
                    //int offLteRadio = SystemProperties.getInt("ril.epsd.lteoff", 1);
                    String isTestSim = SystemProperties.get("gsm.sim.ril.testsim", "0");
                    if (isTestSim.equals("0")){
                        return GeminiNetworkSubUtil.MODE_SIM2_ONLY;
                    } else {
                        // [mmdc] for some test requirement
                        return GeminiNetworkSubUtil.MODE_DUAL_SIM;
                    }
                }
            }
    ...
    protected void setPowerStateToDesired() {
        ...
        //ALPS01035028 - end
            if (!FeatureOption.MTK_GEMINI_SUPPORT) {
                ...
                if(PhoneFactory.isLteDcSupport() && LteModemSwitchHandler.getActiveModemType() == LteModemSwitchHandler.MD_TYPE_LTNG) {
                    // For MMDSDC structure, needs to decide whether turn on protocol1/protocol2 radio 
                    // according to network mode and data usage status
                    int radioMode = getDesiredRadioMode();
                    log("setPowerStateToDesired(): RadioState is OFF, update mPreviousRadioMode="+mPreviousRadioMode+" and setRadioMode to "+ radioMode);
                    if ((radioMode != 0) && (radioMode != mPreviousRadioMode)){
                        mCi.setRadioMode(radioMode, null);
                        mPreviousRadioMode = radioMode;
                    }
                } else {
        ...
        //ALPS01035028 - end
            ...
            if (!FeatureOption.MTK_GEMINI_SUPPORT) {
                ...
                if(PhoneFactory.isLteDcSupport() && LteModemSwitchHandler.getActiveModemType() == LteModemSwitchHandler.MD_TYPE_LTNG) {
                    mPreviousRadioMode = GeminiNetworkSubUtil.MODE_FLIGHT_MODE;
                    log("setPowerStateToDesired(): boot up in Airplane mode, update mPreviousRadioMode="+mPreviousRadioMode+" for flight mode");
                    mCi.setRadioMode(GeminiNetworkSubUtil.MODE_FLIGHT_MODE, null);
                } else {
        ...
        //MTK-STATE:[ALPS01427971] fix set DUAL_SIM_MODE to wrong value in IPO shutdown flow
        if(PhoneFactory.isLteDcSupport() && LteModemSwitchHandler.getActiveModemType() == LteModemSwitchHandler.MD_TYPE_LTNG) {
            setRadioPowerForDataUsageChange();
        }
    ...
    protected void hangupAndPowerOff() {
        ...
        if (!FeatureOption.MTK_GEMINI_SUPPORT) {
            if(PhoneFactory.isLteDcSupport() && LteModemSwitchHandler.getActiveModemType() == LteModemSwitchHandler.MD_TYPE_LTNG) {
                mPreviousRadioMode = GeminiNetworkSubUtil.MODE_FLIGHT_MODE;
                log("hangupAndPowerOff():update mPreviousRadioMode="+mPreviousRadioMode+" for hangupAndPowerOff");
                mCi.setRadioMode(GeminiNetworkSubUtil.MODE_FLIGHT_MODE, null);
            } else {
                mCi.setRadioPower(false, null);
            }
...
*/
    ...
    private void pollStateDone() {
        if (PhoneFactory.isLteDcSupport() && LteModemSwitchHandler.getActiveModemType() == LteModemSwitchHandler.MD_TYPE_LTNG){
            log("MD type is LTNG, no need to sync. CS and PS network type");
        } else {
            ...
            if (mIs3GTo2G == true) {
                //[MMDC]
                if (PhoneFactory.isLteDcSupport() && (this instanceof LteDcServiceStateTracker)){
                    log("ignore to update newps_networkType");
                } else {
...
////        boolean hasRadioTechnologyChanged = mRilRadioTechnology != mNewRilRadioTechnology;
        ...
        if (hasRilVoiceRadioTechnologyChanged) {
            ...
            //}
            updateCsNetworkType(mSimId, Integer.toString(mNewSS.getRilVoiceRadioTechnology()));
           //MTK - END : For Lte Dc
        ...
        if (hasChanged) {
            ...
            prevOperatorNumeric = getOperatorNumericBySim(mSimId);
            updateOperatorAlpha(mSimId, mSS.getOperatorAlphaLong());
            updateOperatorNumeric(mSimId, mSS.getOperatorNumeric());
            updateOperatorIsRoaming(mSimId, mSS.getRoaming());
            //MTK - END : For Lte Dc	
            ...
            //[ALPS01416062] MTK ADD-END
                ...
                //}
                updateOperatorIsoCountry(mSimId, "");
                //MTK - END : For Lte Dc
                ...
                //}
                updateOperatorIsoCountry(mSimId, iso);
                //MTK - END : For Lte Dc
        ...
        if (hasRoamingOn) {
            ...
            //}
            updateRoamingIndicatorNeeded(mSimId, true);
            //MTK - END : For Lte Dc
        ...
        if (hasRoamingOff) {
            ...
            //}
            updateRoamingIndicatorNeeded(mSimId, false);
            //MTK - END : For Lte Dc
...
*/    
    ...
    private void onRestrictedStateChanged(AsyncResult ar) {
        ...
        if (ar.exception == null) {
            ...
            if (PhoneFactory.isLteDcSupport() && LteModemSwitchHandler.getActiveModemType() == LteModemSwitchHandler.MD_TYPE_LTNG
                && PsArbitrator.getIsSwitching()) {
                log("PS switching, ignore PS restrict notification");
            } else {
    ...
    private boolean regCodeIsRoaming (int code) {
        ...
        log("numeric:"+numeric+"mccmnc:"+mccmnc);
...
////        return 5 == code;
    ...
    public boolean isConcurrentVoiceAndDataAllowed() {
        ...
        if (PhoneFactory.isLteDcSupport()) {
            // TODO: check PS arbitrator to see current is dual talk or single talk
            isAllowed = true;            
        }
    ...
    private void setTimeFromNITZString (String nitz, long nitzReceiveTime) {
        ...
        try {
            ...
            //}
            iso = getOperatorIsoCountry(mSimId);
            //MTK - END : For Lte Dc
            ...
            try {
                ...
                if (getAutoTime()) {
                    ...
                    log("NITZ: after Setting time of day");
                }
...
/*
    ...
    protected void log(String s) {
        if(PhoneFactory.isLteDcSupport() && (this instanceof LteDcServiceStateTracker)){
            Rlog.d(LOG_TAG, "[LteDcSST" + mSimId + "] " + s);
        } else {
...
//MTK-START [mtk03851][111124]
    public void setRadioPowerOn() {
        ...
        if (!FeatureOption.MTK_GEMINI_SUPPORT) {
            ...
            /* ALPS00447303 */
            loge("Set mDesiredPowerState in setRadioPowerOn. simMode="+simMode+",airplaneMode="+airplaneMode);
            mDesiredPowerState = (simMode > 0) && (! (airplaneMode > 0));                        			
        }
        /* for consistent UI ,SIM Management for single sim project  END*/
        else {
        ...
        loge("Final mDesiredPowerState in setRadioPowerOn. [" + mDesiredPowerState + "] airplaneMode=" + airplaneMode);
    ...
    public void refreshSpnDisplay() {
        ...
        if (numeric != null) {
	      ...
	      //if (mSimId == PhoneConstants.GEMINI_SIM_1) {
            ...
            //}
            updateOperatorAlpha(mSimId, newAlphaLong);
            //MTK-END for Lte Dc
        } else {
            force = true;
        }

        log("refreshSpnDisplay set mSimId=" +mSimId+","+newAlphaLong +","+newAlphaShort+","+numeric);
    ...
    protected void updateSpnDisplay(boolean forceUpdate) {
        ...
        //ALPS01035028 - start
        log("updateSpnDisplay mVoiceCapable=" + mVoiceCapable + "mEmergencyOnly="+mEmergencyOnly+" mRadioState.isOn()="+mRadioState.isOn()+" getVoiceRegState()="+mSS.getVoiceRegState()+" getDataRegState()"+mSS.getDataRegState());
        //ALPS01035028 - end
        ...
        //MTK-ADD START : for CS not registered , PS regsitered (ex: LTE PS only mode or 2/3G PS only SIM card or CS domain network registeration temporary failure
        if (mVoiceCapable && mEmergencyOnly && mRadioState.isOn() && (mSS.getDataRegState() != ServiceState.STATE_IN_SERVICE)) {
        ...
        */
        ...
        if (showPlmn != mCurShowPlmn
            ...
            //MTK-START : For Lte Dc
            //Intent intent = new Intent(Intents.SPN_STRINGS_UPDATED_ACTION);
            Intent intent = createSpnUpdateIntent();
            //MTK-END : For Lte Dc
            intent.putExtra(PhoneConstants.GEMINI_SIM_ID_KEY, mSimId);
    ...
    public void registerForPsRegistrants(Handler h, int what, Object obj) {
        log("[DSAC DEB] " + "registerForCsRegistrants");
    ...
    public void registerForRatRegistrants(Handler h, int what, Object obj) {
        log("[DSAC DEB] " + "registerForRatRegistrants");
        Registrant r = new Registrant(h, what, obj);
...
//MTK-END [mtk03851][111124]
    ...
    protected int updateOperatorAlpha(int mSimId, String operatorAlphaLong){
        if (mSimId == PhoneConstants.GEMINI_SIM_1) {
            ...
            mPhone.setSystemProperty(TelephonyProperties.PROPERTY_OPERATOR_ALPHA, operatorAlphaLong);
        } else if (mSimId == PhoneConstants.GEMINI_SIM_2){
            log("update PROPERTY_OPERATOR_ALPHA_2 to "+operatorAlphaLong);
            mPhone.setSystemProperty(TelephonyProperties.PROPERTY_OPERATOR_ALPHA_2, operatorAlphaLong);
        } else if (mSimId == PhoneConstants.GEMINI_SIM_3){
            log("update PROPERTY_OPERATOR_ALPHA_3 to "+operatorAlphaLong);
            mPhone.setSystemProperty(TelephonyProperties.PROPERTY_OPERATOR_ALPHA_3, operatorAlphaLong);
        } else if (mSimId == PhoneConstants.GEMINI_SIM_4){
            log("update PROPERTY_OPERATOR_ALPHA_4 to "+operatorAlphaLong);
            mPhone.setSystemProperty(TelephonyProperties.PROPERTY_OPERATOR_ALPHA_4, operatorAlphaLong);
        }
        return 1;
    } 
    ...
    protected int updateOperatorNumeric(int mSimId, String operatorNumeric){
        if (mSimId == PhoneConstants.GEMINI_SIM_1) {
            log("update PROPERTY_OPERATOR_NUMERIC to "+operatorNumeric);
            mPhone.setSystemProperty(TelephonyProperties.PROPERTY_OPERATOR_NUMERIC, operatorNumeric);
        } else if (mSimId == PhoneConstants.GEMINI_SIM_2){
            log("update PROPERTY_OPERATOR_NUMERIC_2 to "+operatorNumeric);
            mPhone.setSystemProperty(TelephonyProperties.PROPERTY_OPERATOR_NUMERIC_2, operatorNumeric);
        } else if (mSimId == PhoneConstants.GEMINI_SIM_3){
            log("update PROPERTY_OPERATOR_NUMERIC_3 to "+operatorNumeric);
            mPhone.setSystemProperty(TelephonyProperties.PROPERTY_OPERATOR_NUMERIC_3, operatorNumeric);
        } else if (mSimId == PhoneConstants.GEMINI_SIM_4){
            log("update PROPERTY_OPERATOR_NUMERIC_4 to "+operatorNumeric);
            mPhone.setSystemProperty(TelephonyProperties.PROPERTY_OPERATOR_NUMERIC_4, operatorNumeric);
        }
        return 1;
    } 
    ...
    protected int updateOperatorIsRoaming(int mSimId, boolean isRoaming){
        if (mSimId == PhoneConstants.GEMINI_SIM_1) {
            log("update PROPERTY_OPERATOR_ISROAMING to "+ (isRoaming ? "true" : "false"));
            mPhone.setSystemProperty(TelephonyProperties.PROPERTY_OPERATOR_ISROAMING, (isRoaming ? "true" : "false"));
        } else if (mSimId == PhoneConstants.GEMINI_SIM_2){
            log("update PROPERTY_OPERATOR_ISROAMING_2 to "+(isRoaming ? "true" : "false"));
            mPhone.setSystemProperty(TelephonyProperties.PROPERTY_OPERATOR_ISROAMING_2, (isRoaming ? "true" : "false"));
        } else if (mSimId == PhoneConstants.GEMINI_SIM_3){
            log("update PROPERTY_OPERATOR_ISROAMING_3 to "+(isRoaming ? "true" : "false"));
            mPhone.setSystemProperty(TelephonyProperties.PROPERTY_OPERATOR_ISROAMING_3, (isRoaming ? "true" : "false"));
        } else if (mSimId == PhoneConstants.GEMINI_SIM_4){
            log("update PROPERTY_OPERATOR_ISROAMING_4 to "+(isRoaming ? "true" : "false"));
            mPhone.setSystemProperty(TelephonyProperties.PROPERTY_OPERATOR_ISROAMING_4, (isRoaming ? "true" : "false"));
        }
        return 1;
    }    
    ...
    protected int updateOperatorIsoCountry(int mSimId, String operatorIsoCountry){
        if (mSimId == PhoneConstants.GEMINI_SIM_1) {
            log("update PROPERTY_OPERATOR_ISO_COUNTRY to "+operatorIsoCountry);
            mPhone.setSystemProperty(TelephonyProperties.PROPERTY_OPERATOR_ISO_COUNTRY, operatorIsoCountry);
        } else if (mSimId == PhoneConstants.GEMINI_SIM_2){
            log("update PROPERTY_OPERATOR_ISO_COUNTRY_2 to "+operatorIsoCountry);
            mPhone.setSystemProperty(TelephonyProperties.PROPERTY_OPERATOR_ISO_COUNTRY_2, operatorIsoCountry);
        } else if (mSimId == PhoneConstants.GEMINI_SIM_3){
            log("update PROPERTY_OPERATOR_ISO_COUNTRY_3 to "+operatorIsoCountry);
            mPhone.setSystemProperty(TelephonyProperties.PROPERTY_OPERATOR_ISO_COUNTRY_3, operatorIsoCountry);
        } else if (mSimId == PhoneConstants.GEMINI_SIM_4){
            log( "update PROPERTY_OPERATOR_ISO_COUNTRY_4 to "+operatorIsoCountry);
            mPhone.setSystemProperty(TelephonyProperties.PROPERTY_OPERATOR_ISO_COUNTRY_4, operatorIsoCountry);
        }
        return 1;
    } 
    ...
    protected int updateDataNetworkType(int mSimId, String dataNetworkType){
        if (mSimId == PhoneConstants.GEMINI_SIM_1) {
            log("update PROPERTY_DATA_NETWORK_TYPE to "+dataNetworkType);
            mPhone.setSystemProperty(TelephonyProperties.PROPERTY_DATA_NETWORK_TYPE, dataNetworkType);
        } else if (mSimId == PhoneConstants.GEMINI_SIM_2){
            log("update PROPERTY_DATA_NETWORK_TYPE_2 to "+dataNetworkType);
            mPhone.setSystemProperty(TelephonyProperties.PROPERTY_DATA_NETWORK_TYPE_2, dataNetworkType);
        } else if (mSimId == PhoneConstants.GEMINI_SIM_3){
            log("update PROPERTY_DATA_NETWORK_TYPE_3 to "+dataNetworkType);
            mPhone.setSystemProperty(TelephonyProperties.PROPERTY_DATA_NETWORK_TYPE_2, dataNetworkType);
        } else if (mSimId == PhoneConstants.GEMINI_SIM_4){
            log("update PROPERTY_DATA_NETWORK_TYPE_4 to "+dataNetworkType);
            mPhone.setSystemProperty(TelephonyProperties.PROPERTY_DATA_NETWORK_TYPE_2, dataNetworkType);
        }
        return 1;
    }
    ...
    protected int updateCsNetworkType(int mSimId, String csNetworkType){
        if (mSimId == PhoneConstants.GEMINI_SIM_1) {
            log("update PROPERTY_CS_NETWORK_TYPE to "+csNetworkType);
            mPhone.setSystemProperty(TelephonyProperties.PROPERTY_CS_NETWORK_TYPE, csNetworkType);
        } else if (mSimId == PhoneConstants.GEMINI_SIM_2){
            log("update PROPERTY_CS_NETWORK_TYPE_2 to "+csNetworkType);
            mPhone.setSystemProperty(TelephonyProperties.PROPERTY_CS_NETWORK_TYPE_2, csNetworkType);
        } else if (mSimId == PhoneConstants.GEMINI_SIM_3){
            log("update PROPERTY_CS_NETWORK_TYPE_3 to "+csNetworkType);
            mPhone.setSystemProperty(TelephonyProperties.PROPERTY_CS_NETWORK_TYPE_3, csNetworkType);
        } else if (mSimId == PhoneConstants.GEMINI_SIM_4){
            log("update PROPERTY_CS_NETWORK_TYPE_4 to "+csNetworkType);
            mPhone.setSystemProperty(TelephonyProperties.PROPERTY_CS_NETWORK_TYPE_4, csNetworkType);
        }
        return 1;
    }    
    ...
    protected int updateRoamingIndicatorNeeded(int mSimId, boolean isNeeded){
        if (mSimId == PhoneConstants.GEMINI_SIM_1) {
            log("update PROPERTY_ROAMING_INDICATOR_NEEDED to "+(isNeeded ? "true" : "false"));
            mPhone.setSystemProperty(TelephonyProperties.PROPERTY_ROAMING_INDICATOR_NEEDED, isNeeded ? "true" : "false");
        } else if (mSimId == PhoneConstants.GEMINI_SIM_2){
            log("update PROPERTY_ROAMING_INDICATOR_NEEDED_2 to "+(isNeeded ? "true" : "false"));
            mPhone.setSystemProperty(TelephonyProperties.PROPERTY_ROAMING_INDICATOR_NEEDED_2, isNeeded ? "true" : "false");
        } else if (mSimId == PhoneConstants.GEMINI_SIM_3){
            log("update PROPERTY_ROAMING_INDICATOR_NEEDED_3 to "+(isNeeded ? "true" : "false"));
            mPhone.setSystemProperty(TelephonyProperties.PROPERTY_ROAMING_INDICATOR_NEEDED_3, isNeeded ? "true" : "false");
        } else if (mSimId == PhoneConstants.GEMINI_SIM_4){
            log("update PROPERTY_ROAMING_INDICATOR_NEEDED_4 to "+(isNeeded ? "true" : "false"));
            mPhone.setSystemProperty(TelephonyProperties.PROPERTY_ROAMING_INDICATOR_NEEDED_4, isNeeded ? "true" : "false");
        }
        return 1;
    }    
    ...
    protected String getOperatorNumericBySim(int mSimId){
        String retStr = "";    
        if (mSimId == PhoneConstants.GEMINI_SIM_1) {
            retStr = SystemProperties.get(TelephonyProperties.PROPERTY_OPERATOR_NUMERIC, "");
        } else if (mSimId == PhoneConstants.GEMINI_SIM_2){
            retStr = SystemProperties.get(TelephonyProperties.PROPERTY_OPERATOR_NUMERIC_2, "");
        } else if (mSimId == PhoneConstants.GEMINI_SIM_3){
            retStr = SystemProperties.get(TelephonyProperties.PROPERTY_OPERATOR_NUMERIC_3, "");
        } else if (mSimId == PhoneConstants.GEMINI_SIM_4){
            retStr = SystemProperties.get(TelephonyProperties.PROPERTY_OPERATOR_NUMERIC_4, "");
        }
        return retStr;
    } 
    ...
    protected String getOperatorIsoCountry(int mSimId){
        String retStr = "";    
        if (mSimId == PhoneConstants.GEMINI_SIM_1) {
            retStr = SystemProperties.get(TelephonyProperties.PROPERTY_OPERATOR_ISO_COUNTRY);
        } else if (mSimId == PhoneConstants.GEMINI_SIM_2){
            retStr = SystemProperties.get(TelephonyProperties.PROPERTY_OPERATOR_ISO_COUNTRY_2);
        } else if (mSimId == PhoneConstants.GEMINI_SIM_3){
            retStr = SystemProperties.get(TelephonyProperties.PROPERTY_OPERATOR_ISO_COUNTRY_3);
        } else if (mSimId == PhoneConstants.GEMINI_SIM_4){
            retStr = SystemProperties.get(TelephonyProperties.PROPERTY_OPERATOR_ISO_COUNTRY_4);
        }
        return retStr;
    }

    protected Intent createSpnUpdateIntent(){
        return new Intent(Intents.SPN_STRINGS_UPDATED_ACTION);
    }

    protected void setCurrentPreferredNwType(){
        if (PhoneFactory.isLteDcSupport() && LteModemSwitchHandler.getActiveModemType() == LteModemSwitchHandler.MD_TYPE_LTNG) {
                Message msg = obtainMessage();
                msg.arg1 = 1; //to avoid sent AT+EFUN (switch radio mode) when excute LteDcPhone().setPreferredNetworkType() on next step.

                //Get preferredNetworkMode from Settings.System
                int mPreferredNwTypeInRil = Settings.Global.getInt(mPhone.getContext().getContentResolver(),
                        Settings.Global.PREFERRED_NETWORK_MODE, RILConstants.PREFERRED_NETWORK_MODE);

                log("setCurrentPreferredNwType to mPreferredNwTypeInRil=" + mPreferredNwTypeInRil);

                PhoneFactory.getLteDcPhone().setPreferredNetworkType(mPreferredNwTypeInRil, msg);
        } else {
            //ALPS00607052 add for AT&T [GSM-BTR-1-7942]            
            if (!FeatureOption.MTK_RAT_BALANCING){
                log("setCurrentPreferredNwType() : FeatureOption.MTK_RAT_BALANCING is False");
                // Set the network type, in case the radio does not restore it.
                mCi.setCurrentPreferredNetworkType();
            }
        }
    ...
    public void setPowerForRatModeChanged(Message response) {
        int radioMode = getDesiredRadioMode();
        log("setPowerForRatModeChanged() RadioMode= "+ radioMode + " mPreviousRadioMode=" +mPreviousRadioMode);
        if ((radioMode != 0) && (radioMode != mPreviousRadioMode)){
            log("setPowerForRatModeChanged() set RadioMode to "+ radioMode);
            if (response != null){
            ...
            if (response != null){
                log("setPowerForRatModeChanged() do nothing");
    ...
    @Override
    public void notifyDataUsageChange() {
        log("notifyDataUsageChange");
        Message msg = obtainMessage(EVENT_DATA_USAGE_CHANGE);
        sendMessage(msg);
    }

    
    //MTK-STATE:[ALPS01427971] fix set DUAL_SIM_MODE to wrong value in IPO shutdown flow
    public void setRadioPowerForDataUsageChange() {
        log("setRadioPowerForDataUsageChange mDesiredPowerState:" + mDesiredPowerState +
                " current radio state:" + mRadioState+
    			" mFirstRadioChange:" + mFirstRadioChange);

        // In MMDSDC structure, 
        // There has two scenario for mDesiredPowerState is true
        // 1. data usage is off -> EFUN = 2
        // 2. data usage is on -> EFUN = 3
        // do special handle when mDesiredPowerState is true, and radio state is off on LteDcManger
        if(PhoneFactory.isLteDcSupport() && 
           (LteModemSwitchHandler.getActiveModemType() == LteModemSwitchHandler.MD_TYPE_LTNG) && 
           (mDesiredPowerState && mCi.getRadioState().isOn())) {
                // For MMDSDC structure, needs to decide whether turn on protocol1/protocol2 radio 
                // according to network mode and data usage status
                int radioMode = getDesiredRadioMode();
                log("setRadioPowerForDataUsageChange():accrording to check data usage status,update mPreviousRadioMode="
                        +mPreviousRadioMode+" and setRadioMode to "+ radioMode);
                if ((radioMode != 0) && (radioMode != mPreviousRadioMode)){
                    mCi.setRadioMode(radioMode, null);
                    mPreviousRadioMode = radioMode;
                }                    
                //mCi.setRadioMode(getDesiredRadioMode(), null);
        }
    }
    //MTK-END:[ALPS01427971] fix set DUAL_SIM_MODE to wrong value in IPO shutdown flow

    //MTK-END  for Lte Dc feature
}
