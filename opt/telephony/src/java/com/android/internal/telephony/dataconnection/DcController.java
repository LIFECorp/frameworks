        ...
        public boolean processMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case DataConnection.EVENT_DATA_STATE_CHANGED:
                    ar = (AsyncResult)msg.obj;
                    if (ar.exception == null && ((ArrayList<DataCallResponse>) ar.result).size() != 0) {
            ...
            for (DataCallResponse newState : dcsList) {
                ...
                if (dc == null) {
                    ...
                    if (newState.active == DATA_CONNECTION_ACTIVE_PH_LINK_UP) {
                        // the state is un-sync between RIL and framework
                        loge("onDataStateChanged: found un-sync PDP cid=" + newState.cid);
                        mPhone.mCi.deactivateDataCall(newState.cid, RILConstants.DEACTIVATE_REASON_PDP_UNSYNC, null);
                    } else {
                        // UNSOL_DATA_CALL_LIST_CHANGED arrived before SETUP_DATA_CALL completed.
                        loge("onDataStateChanged: no associated DC yet, ignore");
                    }           
