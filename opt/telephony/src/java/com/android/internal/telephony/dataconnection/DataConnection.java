    ...
    private class DcRetryingState extends State {
        ...
        public void enter() {
            ...
            if ((mConnectionParams.mRilRat != mRilRat)
                    || (mDataRegState != ServiceState.STATE_IN_SERVICE && !FeatureOption.MTK_GEMINI_SUPPORT)){
                ...
                case EVENT_GET_LAST_FAIL_DONE:
                    ...
                    if (cp.mTag == mTag) {
                        ...
                        if (cause.isRestartRadioFail()) {
                            ...
                            transitionTo(mInactiveState); 
                        } else if (mGsmDCTExt.hasSmCauseRetry(rilFailCause)) {
                            //[SM_CAUSE]
                            if (DBG) log("DcActivatingState: Costumize SM_CAUSE:" + cause );                  
                            mRetryCount++;
                            if (mGsmDCTExt.needRetry(mRetryCount)){
