...
//[MMDC]
//import com.android.internal.telephony.psarbitrator.PsArbitrator;
...
//public final class RIL extends BaseCommands implements CommandsInterface {
    ...
    String SOCKET_NAME_RIL_1 = "rild";
    String SOCKET_NAME_RIL_2 = "rild2";
    //MTK-END [mtk04070][111121][ALPS00093395]For supporting Gemini
    //MTK-START For supporting Gemini+
    String SOCKET_NAME_RIL_3 = "rild3";
    String SOCKET_NAME_RIL_4 = "rild4";
    ...
    private LteDcManager mLteDcManager = null;
    private boolean mNotifyStatusMsgAllow = true;
    //MTK-START [MMDC]
    HandlerThread mArbitratorThread;
    public static RilPsArbitrator mRilPsArbitrator;

    HandlerThread mPsArbitratorThread;
    public static PsArbitrator mPsArbitrator;    
    //MTK-END [MMDC]
    ...
    //[MMDC] Ril Arbitrator, connect to PsArbitrator module which deal with data domain switch
    public RilPsArbitrator getRilPsArbitrator(){
        if (PhoneFactory.isLteDcSupport()){
            if (mRilPsArbitrator == null && !(this instanceof LteDcManager)){            
                mArbitratorThread = new HandlerThread("RilArbitrator");
                mArbitratorThread.start();
                
                Looper rilPslooper = mArbitratorThread.getLooper();
                mRilPsArbitrator = new RilPsArbitrator(rilPslooper);            
            }

            return mRilPsArbitrator;
        }        
        return null;
    }    
    
    public class RilPsArbitrator extends Handler implements Runnable{
        private static final String LOG_TAG = "RilPsArbitrator";
        
        //private PsArbitrator mPsArbitrator;
        public int mDecisionMode;
        public int mCurrentMode;

        //queue the RR
        public ArrayList<RILRequest> mPsRequestList = new ArrayList<RILRequest>();        

        public RilPsArbitrator(Looper looper) {
            super(looper);

            //new the PsArbitrator
            RIL.this.getPsArbitrator();
            

            //  Initial Mode 
            if (mPsArbitrator != null){
                mDecisionMode = mPsArbitrator.getDecisionPsMode();
                mCurrentMode = mPsArbitrator.getCurrentPsMode();
                riljLog("[RilPsArbitrator] PsArbitrtor initialization completed");
            } else {
                riljLog("[RilPsArbitrator] Initialize PsArbitrtor failed");
                mDecisionMode = PsArbitrator.PS_MODE_NONE;
                mCurrentMode = PsArbitrator.PS_MODE_NONE;
            }
        ...
        public void handleMessage (Message msg) {
            riljLog("[RilPsArbitrator] handleMessage msg=" + msg);


            //Temp no need handle Msgs
            switch (msg.what) {

                /*
                case PsArbitrator.EVENT_PS_SWITCH:
                    Log.d(LOG_TAG, "RilsArbitrator.EVENT_PS_SWITCH: Start");                    
                    break;
                    
                case PsArbitrator.EVENT_PS_SWITCH_DONE:
                    //this event from the +EPSD conf ok
                    Log.d(LOG_TAG, "RilsArbitrator.EVENT_PS_SWITCH_DONE: pop msg and execute");
                    RIL.this.executeMmdcDataCmdsQueue();                    
                    break;*/
        ...
        //dispatch to different protocol
        public void send(RILRequest rr) {
            // mDecisionMode will be updated by modem-psdm
            // then mCurrentMode will be updated after PsArbitrator.onDecisionChanged trigger switch
            ...
            switch (rr.mRequest) {
                ...
                case RIL_REQUEST_SET_GPRS_CONNECT_TYPE:
                    ...
                    if (mDecisionMode == PsArbitrator.PS_MODE_GSM) {
                        riljLog("[RilPsArbitrator] The decision is 2G, send the request " + requestToString(rr.mRequest));
                        RIL.this.send(rr);
                    } else if (mDecisionMode == PsArbitrator.PS_MODE_UMTS|| mDecisionMode == PsArbitrator.PS_MODE_LTE) {
                        riljLog("[RilPsArbitrator] The decision is 3G or 4G, send the request " + requestToString(rr.mRequest));
                        getLteDcManager().send(rr);
                    } else {
                        //Error: No decision will not into this "send"
                        riljLog("[RilPsArbitrator] The decision is N/A, not to send the request " + requestToString(rr.mRequest));
                    }
                    break;
                ...
                //case RIL_REQUEST_SET_FD_MODE:
                    if (mCurrentMode == PsArbitrator.PS_MODE_GSM) {
                        riljLog("[RilPsArbitrator] The current mode is 2G, send the request :" + requestToString(rr.mRequest));
                        RIL.this.send(rr);
                    } else if (mCurrentMode == PsArbitrator.PS_MODE_UMTS || mCurrentMode == PsArbitrator.PS_MODE_LTE) {
                        riljLog("[RilPsArbitrator] The current mode is 3G or 4G, send the request :" + requestToString(rr.mRequest));
                        getLteDcManager().send(rr);
                    } else {
                        //Error: No decision will not into this "send"
                        riljLog("[RilPsArbitrator] The current mode is N/A, not to send the request :" + requestToString(rr.mRequest));
                    }
                    break;

                case RIL_REQUEST_DATA_CALL_LIST:
                case RIL_REQUEST_DATA_REGISTRATION_STATE:
                    //to record the requeset is send to which protocol
                    rr.mResult.arg2 = mCurrentMode;
                    ...
                    if (mCurrentMode == PsArbitrator.PS_MODE_GSM) {
                        ...
                        //Error: No decision will not into this "send"
                        riljLog("[RilPsArbitrator] The current mode is N/A, send the request to protocol 2 :" + requestToString(rr.mRequest));
                        RIL.this.send(rr);
                    }                    
                    break;
                case RIL_REQUEST_PS_REATTACH:
                    if (mCurrentMode == PsArbitrator.PS_MODE_UMTS || mCurrentMode == PsArbitrator.PS_MODE_LTE) {
                        riljLog("[RilPsArbitrator] The current mode is 3/4G, send the request to protocol 1 :" + requestToString(rr.mRequest)); 
                        getLteDcManager().send(rr);                    
                    } else {
                        riljLog("[RilPsArbitrator] ignore:" + requestToString(rr.mRequest));
                    }
        ...
        public int getDecisionMode(){
            mDecisionMode = mPsArbitrator.getDecisionPsMode();
            return mDecisionMode;
        }


        //clear queue 
        public void clearPsRequestQueue(){

            mRilPsArbitrator.mPsRequestList.clear();

        }

        public void doMmdcDataCmds(RILRequest rr){

            if(mRilPsArbitrator.mDecisionMode == PsArbitrator.PS_MODE_NONE
                || mPsArbitrator.getIsSwitching()){
                // no decision or is switching
                riljLog("[RilPsArbitrator] mRilPsArbitrator.mDecisionMode:" + mRilPsArbitrator.mDecisionMode + " and PsArbitrator is switching:" + mPsArbitrator.getIsSwitching());
                synchronized(mRilPsArbitrator.mPsRequestList){
                    riljLog("[RilPsArbitrator] Ps is Switching or no decision, enqueue request: " + RIL.requestToString(rr.mRequest));
                    mRilPsArbitrator.mPsRequestList.add(rr);     
                }                      
            } else {
                // dispatch to right protocol, dispatch directly
                mRilPsArbitrator.send(rr);
            }
            
        }

        public void executeMmdcDataCmdsQueue(){

            riljLog("[RilPsArbitrator] Execute data cmds queue! E");
            riljLog("[RilPsArbitrator] queue size:" + mRilPsArbitrator.mPsRequestList.size() + ", is switching:" + mPsArbitrator.getIsSwitching());
            synchronized(mRilPsArbitrator.mPsRequestList){
                while(mRilPsArbitrator.mPsRequestList.size() != 0 && !mPsArbitrator.getIsSwitching()) {
                    RILRequest rr = mRilPsArbitrator.mPsRequestList.get(0);
                    mRilPsArbitrator.mPsRequestList.remove(0);
                    mRilPsArbitrator.send(rr);
                }        
            }
            riljLog("[RilPsArbitrator] Execute data cmds queue! X");   
        }
...
/* 3G switch end */
                ...
                mSocket = s;
                riljLog("Connected to '" + socketRil + "' socket");
    ...
    public RIL(Context context, int preferredNetworkType, int cdmaSubscription, int simId) {
        ...
        if (cm.isNetworkSupported(ConnectivityManager.TYPE_MOBILE) == false) {
            riljLog("Not starting RILReceiver: wifi-only");
             ...
             // connect socket directly when it is lte handler,
            if(PhoneFactory.isLteDcSupport() && (this instanceof LteDcManager)) {
                startRilReceiver();
            }
        ...
        // init LteDcManager
        if( PhoneFactory.isLteDcSupport() &&
            !(this instanceof LteDcManager) &&
            mySimId == PhoneConstants.GEMINI_SIM_1) {
            if (RILJ_LOGD) riljLog("new getLteDcManager");
            mLteDcManager = new LteDcManager(mContext, mPreferredNetworkType, mCdmaSubscription);
        }

        //MTK-START [MMDC] inner singleton class initial
        if (PhoneFactory.isLteDcSupport()){
            if (mRilPsArbitrator == null && !(this instanceof LteDcManager)){            
                mArbitratorThread = new HandlerThread("RilArbitrator");
                mArbitratorThread.start();
    ...
    private void setSocketNames() {
        ...
        if (telephonyMode == PhoneFactory.MODE_0_NONE) {
            ...
            if(PhoneFactory.isLteDcSupport() && mySimId == PhoneConstants.GEMINI_SIM_1) {
                if(this instanceof LteDcManager) {
                    // LT protocol
                    SOCKET_NAME_RIL_1 = "rild";
                    SOCKET_NAME_RIL_2 = "";
                } else {
    ...
    getIccCardStatus(Message result) {
        ...
        if(PhoneFactory.isLteDcSupport() && !(this instanceof LteDcManager)) {
            getLteDcManager().getIccCardStatus(result);
        } else {
    ...
    supplyIccPinForApp(String pin, String aid, Message result) {
        ...
        if(PhoneFactory.isLteDcSupport() && !(this instanceof LteDcManager)) {
            getLteDcManager().supplyIccPinForApp(pin, aid, result);
        } else {
    ...
    supplyIccPukForApp(String puk, String newPin, String aid, Message result) {
        ...
        //       but this request is also valid for SIM and RUIM
        if(PhoneFactory.isLteDcSupport() && !(this instanceof LteDcManager)) {
            getLteDcManager().supplyIccPukForApp(puk, newPin, aid, result);
        } else {
    ...
    supplyIccPin2ForApp(String pin, String aid, Message result) {
        ...
        //       but this request is also valid for SIM and RUIM
        if(PhoneFactory.isLteDcSupport() && !(this instanceof LteDcManager)) {
            getLteDcManager().supplyIccPin2ForApp(pin, aid, result);
        } else {
    ...
    supplyIccPuk2ForApp(String puk, String newPin2, String aid, Message result) {
        ...
        //       but this request is also valid for SIM and RUIM
        if(PhoneFactory.isLteDcSupport() && !(this instanceof LteDcManager)) {
            getLteDcManager().supplyIccPuk2ForApp(puk, newPin2, aid, result);
        } else {
    ...
    changeIccPinForApp(String oldPin, String newPin, String aid, Message result) {
        ...
        //       but this request is also valid for SIM and RUIM
        if(PhoneFactory.isLteDcSupport() && !(this instanceof LteDcManager)) {
            getLteDcManager().changeIccPinForApp(oldPin, newPin, aid, result);
        } else {
    ...
    changeIccPin2ForApp(String oldPin2, String newPin2, String aid, Message result) {
        ...
        //       but this request is also valid for SIM and RUIM
        if(PhoneFactory.isLteDcSupport() && !(this instanceof LteDcManager)) {
            getLteDcManager().changeIccPin2ForApp(oldPin2, newPin2, aid, result);
        } else {
    ...
    getDataCallList(Message result) {
        ...
        if(PhoneFactory.isLteDcSupport() && LteModemSwitchHandler.getActiveModemType() == LteModemSwitchHandler.MD_TYPE_LTNG){
            //mRilPsArbitrator.doMmdcDataCmds(rr);
            mRilPsArbitrator.send(rr);
        } else {                
            send(rr);
        }
    ...
    getIMSIForApp(String aid, Message result) {
    	  if(PhoneFactory.isLteDcSupport() && !(this instanceof LteDcManager)) {
            getLteDcManager().getIMSIForApp(aid, result);
        } else {
    ...
    setRadioPower(boolean on, Message result) {
        ...
        if(PhoneFactory.isLteDcSupport() && !(this instanceof LteDcManager)) {
            getLteDcManager().setRadioPower(on, result);
        } else {
    ...
    iccIOForApp (int command, int fileid, String path, int p1, int p2, int p3,
        ...
        if(PhoneFactory.isLteDcSupport() && !(this instanceof LteDcManager)) {
            getLteDcManager().iccIOForApp(command, fileid, path, p1, p2, p3, data, pin2, null, result);
        } else {
        //Note: This RIL request has not been renamed to ICC,
        //       but this request is also valid for SIM and RUIM
        RILRequest rr
    ...
    queryFacilityLockForApp(String facility, String password, int serviceClass, String appId,
                            Message response) {
        if(PhoneFactory.isLteDcSupport() && !(this instanceof LteDcManager)) {
            getLteDcManager().queryFacilityLockForApp(facility, password, serviceClass, appId, response);
        } else {  
    ...
    setFacilityLockForApp(String facility, boolean lockState, String password,
        ...
        if(PhoneFactory.isLteDcSupport() && !(this instanceof LteDcManager)) {
            getLteDcManager().setFacilityLockForApp(facility, lockState, password, serviceClass, appId, response);
        } else { 
            String lockString;
             RILRequest rr
    ...
    public void setCurrentPreferredNetworkType() {
        if(PhoneFactory.isLteDcSupport() && !(this instanceof LteDcManager)) {
            getLteDcManager().setCurrentPreferredNetworkType();
        } else {    
    ...
    public void setPreferredNetworkType(int networkType , Message response) {
        if(PhoneFactory.isLteDcSupport() && !(this instanceof LteDcManager)) {
            getLteDcManager().setPreferredNetworkType(networkType, response);
        } else {
            int ratOnlyStart = 100;
            RILRequest rr = RILRequest.obtain(
    ...
    public void getPreferredNetworkType(Message response) {
        if(PhoneFactory.isLteDcSupport() && !(this instanceof LteDcManager)) {
            getLteDcManager().getPreferredNetworkType(response);
        } else {    
            RILRequest rr = RILRequest.obtain(
...
////            if ((mIs3GSwitch && mySimId == PhoneConstants.GEMINI_SIM_2) || (!mIs3GSwitch && mySimId == PhoneConstants.GEMINI_SIM_1)) {
    ...
    //[MMDC] temp modify the private-> public
    public void
    send(RILRequest rr) {
        Message msg;
 ...
 | sed -re 's/\{([^,]+),[^,]+,([^}]+).+/case \1: ret = \2(p); break;/'
            ...
            //[MMDC] PSDM
            case RIL_REQUEST_GET_PSDM_DECISION: ret = responseInts(p); break;
            case RIL_REQUEST_CONFIRM_PS_SWITCH: ret = responseVoid(p); break;
            case RIL_REQUEST_CONFIRM_PS_SWITCH_ATTACH: ret = responseVoid(p); break;
            case RIL_REQUEST_RESET_MODE_AFTER_SWITCH : ret = responseVoid(p); break;
            case RIL_REQUEST_PS_REATTACH: ret = responseVoid(p); break;           
...
*/
            ...
            //[MMDC] PSDM
            case RIL_UNSOL_PSDM_DECISION_CHANGED: ret = responseInts(p); break;
            case RIL_UNSOL_SPECIAL_BAND_CHANGED: ret = responseInts(p); break;
            default:
        ...
        switch(response) {
            ...
            case RIL_UNSOL_DATA_CALL_LIST_CHANGED:
                if (RILJ_LOGD) unsljLogRet(response, ret);
                //[MMDC] for registerForDataNetworkStateChanged: DcController, DcTracker
                if (PhoneFactory.isLteDcSupport() && LteModemSwitchHandler.getActiveModemType() == LteModemSwitchHandler.MD_TYPE_LTNG){
                    int reporter = (this instanceof LteDcManager) ? 0 : 1; //LteManager: 0, Ril: 1
                    int user = (mPsArbitrator.getCurrentPsMode() == PsArbitrator.PS_MODE_UMTS                        
                                || mPsArbitrator.getCurrentPsMode() == PsArbitrator.PS_MODE_LTE) ? 0 : 1;
                    if (RILJ_LOGD) riljLog("UNSOL_DATA_CALL_LIST_CHANGED");
                    if (RILJ_LOGD) riljLog("Reporter:" + reporter + ", User:" + user);
                    //if current data side, then notify
                    if (reporter == user){
                        getPsArbitrator().getRil().mDataNetworkStateRegistrants.notifyRegistrants(new AsyncResult(null, ret, null));                        
                    } else {
                        if (RILJ_LOGD) riljLog("ignore UNSOL_DATA_CALL_LIST_CHANGED");
                    }
                } else {
            ...
            case RIL_UNSOL_PSDM_DECISION_CHANGED:
                ...
                if (mPsdmDecisionRegistrants != null){
                    mPsdmDecisionRegistrants.notifyRegistrants(new AsyncResult(null, ret, null));
                }
            ...
            case RIL_UNSOL_GPRS_DETACH:
                ...
                if (PhoneFactory.isLteDcSupport() && LteModemSwitchHandler.getActiveModemType() == LteModemSwitchHandler.MD_TYPE_LTNG){
                    ...
                    int user = (mPsArbitrator.getCurrentPsMode() == PsArbitrator.PS_MODE_UMTS                        
                                || mPsArbitrator.getCurrentPsMode() == PsArbitrator.PS_MODE_LTE) ? 0 : 1;                    
                    if (RILJ_LOGD) riljLog("RIL_UNSOL_GPRS_DETACH");
                    if (RILJ_LOGD) riljLog("Reporter:" + reporter + ", User:" + user);
                    //if current data side, then notify
                    if (getPsArbitrator().getRil()!= null) {
                        getPsArbitrator().getRil().mGprsDetachRegistrant.notifyRegistrant(new AsyncResult(null, ret, null));
                    } else {                    
                        if (RILJ_LOGD) riljLog("ignore RIL_UNSOL_GPRS_DETACH");
                    }
                } else {
            ...
            case RIL_UNSOL_RESPONSE_PS_NETWORK_STATE_CHANGED:
                ...
                if (PhoneFactory.isLteDcSupport() && LteModemSwitchHandler.getActiveModemType() == LteModemSwitchHandler.MD_TYPE_LTNG){
                    ...
                    int user = (mPsArbitrator.getCurrentPsMode() == PsArbitrator.PS_MODE_UMTS                        
                                || mPsArbitrator.getCurrentPsMode() == PsArbitrator.PS_MODE_LTE) ? 0 : 1;
                    if (RILJ_LOGD) riljLog("RIL_UNSOL_RESPONSE_PS_NETWORK_STATE_CHANGED");
                    if (RILJ_LOGD) riljLog("Reporter:" + reporter + ", User:" + user);
                    //if current data side, then notify
                    if (getPsArbitrator().getRil()!= null && (reporter == user)) {
                        getPsArbitrator().getRil().mPsNetworkStateRegistrants
                            .notifyRegistrants(new AsyncResult(null, null, null));
                    } else {
                        if (RILJ_LOGD) riljLog("ignore: RIL_UNSOL_RESPONSE_PS_NETWORK_STATE_CHANGED");
                    }
                } else {
...
*/
        switch(request) {
            ...
            //[MMDC] PSDM
            case RIL_REQUEST_GET_PSDM_DECISION: return "RIL_REQUEST_GET_PSDM_DECISION";
            case RIL_REQUEST_CONFIRM_PS_SWITCH: return "RIL_REQUEST_CONFIRM_PS_SWITCH";          
            case RIL_REQUEST_CONFIRM_PS_SWITCH_ATTACH: return "RIL_REQUEST_CONFIRM_PS_SWITCH_ATTACH";
            case RIL_REQUEST_RESET_MODE_AFTER_SWITCH: return "RIL_REQUEST_RESET_MODE_AFTER_SWITCH";
            case RIL_REQUEST_PS_REATTACH: return "RIL_REQUEST_PS_REATTACH";
...
*/
        switch(request) {
            ...
            case RIL_UNSOL_PSDM_DECISION_CHANGED: return "UNSOL_PSDM_DECISION_CHANGED";//[MMDC]
            case RIL_UNSOL_SPECIAL_BAND_CHANGED: return "RIL_UNSOL_SPECIAL_BAND_CHANGED";//[MMDC]
    ...
    iccExchangeAPDU(int cla, int command, int channel, int p1, int p2, int p3,
        ...
        if(PhoneFactory.isLteDcSupport() && !(this instanceof LteDcManager)) {
            getLteDcManager().iccExchangeAPDU(cla, command, channel, p1, p2, p3, data, result);
        } else {
            RILRequest rr;
            if (channel == 0) {
    ...
    iccOpenChannel(String AID, Message result) {
        if(PhoneFactory.isLteDcSupport() && !(this instanceof LteDcManager)) {
            getLteDcManager().iccOpenChannel(AID, result);
        } else {
            RILRequest rr
    ...
    iccCloseChannel(int channel, Message result) {
        if(PhoneFactory.isLteDcSupport() && !(this instanceof LteDcManager)) {
            getLteDcManager().iccCloseChannel(channel, result);
        } else {
            RILRequest rr
    ...
    iccGetATR(Message result) {
        if(PhoneFactory.isLteDcSupport() && !(this instanceof LteDcManager)) {
            getLteDcManager().iccGetATR(result);
        } else {
    ...
    iccOpenChannelWithSw(String AID, Message result) {
        if(PhoneFactory.isLteDcSupport() && !(this instanceof LteDcManager)) {
            getLteDcManager().iccOpenChannelWithSw(AID, result);
        } else {
    ...
    */
    ...
    setRadioPowerOff(Message result) {
        if(PhoneFactory.isLteDcSupport() && !(this instanceof LteDcManager)) {
            getLteDcManager().setRadioPowerOff(result);
        } else {
    ...
    setRadioPowerOn(Message result) {
        if(PhoneFactory.isLteDcSupport() && !(this instanceof LteDcManager)) {
            getLteDcManager().setRadioPowerOn(result);
        } else {
    ...
    setRadioMode(int mode, Message result) {
        if(PhoneFactory.isLteDcSupport() && !(this instanceof LteDcManager)) {
            getLteDcManager().setRadioMode(mode, result);
        } else {
    ...
    public void sendBTSIMProfile(int nAction, int nType, String strData, Message response) {
        if(PhoneFactory.isLteDcSupport() && !(this instanceof LteDcManager)) {
            getLteDcManager().sendBTSIMProfile(nAction, nType, strData, response);
        } else {
    ...
    */
    ...
    public void queryIccId(Message result){
        if (PhoneFactory.isLteDcSupport() && !(this instanceof LteDcManager)) {
            getLteDcManager().queryIccId(result);
        } else {
    ...
    public void setTRM(int mode, Message result) {
        if(PhoneFactory.isLteDcSupport() && !(this instanceof LteDcManager)) {
            getLteDcManager().setTRM(mode, result);
        } else {
    ...
    public boolean isGettingAvailableNetworks() {
        synchronized (mRequestList) {
            for (int i = 0, s = mRequestList.size() ; i < s ; i++) {
                ...
                if (rr != null && rr.mRequest == RIL_REQUEST_QUERY_AVAILABLE_NETWORKS) {
                    return true;
                }
            }
        }
    ...
    @Override
    public void
    getPsdmDecision(Message result){
        //AT+EPSD=?
        //+EPSD: <current rat>, <target rat>
        if (PhoneFactory.isLteDcSupport() && LteModemSwitchHandler.getActiveModemType() == LteModemSwitchHandler.MD_TYPE_LTNG){
            RILRequest rr = RILRequest.obtain(RIL_REQUEST_GET_PSDM_DECISION, result, mySimId);
    ...
    @Override
    public void confirmPsSwitch(int apDecision, int targerRat, Message result){
        //AT+EPDS = <apDecision>, <targetRat>
        if (PhoneFactory.isLteDcSupport() && LteModemSwitchHandler.getActiveModemType() == LteModemSwitchHandler.MD_TYPE_LTNG){
            RILRequest rr = RILRequest.obtain(RIL_REQUEST_CONFIRM_PS_SWITCH, result, mySimId);

            rr.mParcel.writeInt(2);
            rr.mParcel.writeInt(apDecision);
            rr.mParcel.writeInt(targerRat);

            if (RILJ_LOGD) riljLog(rr.serialString() + "> " + requestToString(rr.mRequest) + ": " + apDecision + ", "+ targerRat);

            riljLog("confirmPs to Protocol: " + getPsArbitrator().getDispatchProtocol());
            if(getPsArbitrator().getDispatchProtocol() == PsArbitrator.PS_PROTOCOL_1){
                mLteDcManager.send(rr);
            } else { 
                send(rr);    
            }
        }
    ...
    @Override
    public void confirmPsSwitchAttach(int protocol, Message result){
        //AT+EGTYPE=2, AT+CGATT=1
        if (PhoneFactory.isLteDcSupport() && LteModemSwitchHandler.getActiveModemType() == LteModemSwitchHandler.MD_TYPE_LTNG){
            RILRequest rr = RILRequest.obtain(RIL_REQUEST_CONFIRM_PS_SWITCH_ATTACH, result, mySimId);        

            rr.mParcel.writeInt(1);
            rr.mParcel.writeInt(protocol);

            if (RILJ_LOGD) riljLog(rr.serialString() + "> " + requestToString(rr.mRequest) + ": " + protocol);

            if(protocol == PsArbitrator.PS_PROTOCOL_2){
                send(rr);
            } else {
                mLteDcManager.send(rr);
            }
    ...
    @Override
    public void resetModeAfterSwitch(int protocol, Message result){
        //AT+EGTYPE=0
        if (PhoneFactory.isLteDcSupport() && LteModemSwitchHandler.getActiveModemType() == LteModemSwitchHandler.MD_TYPE_LTNG){
            RILRequest rr = RILRequest.obtain(RIL_REQUEST_RESET_MODE_AFTER_SWITCH, result, mySimId);        
    ...
    protected void requestPsReattach(Message result){
        if (PhoneFactory.isLteDcSupport() && LteModemSwitchHandler.getActiveModemType() == LteModemSwitchHandler.MD_TYPE_LTNG){
            RILRequest rr = RILRequest.obtain(RIL_REQUEST_PS_REATTACH, result, mySimId);
    ...
    public void SyncNotifyDataCallList(AsyncResult ret){
        riljLog("[RilPsArbitrator] notify data call list!"); 
        mDataNetworkStateRegistrants.notifyRegistrants(ret);
    }

    // override
    public PsArbitrator getPsArbitrator() {

        if (PhoneFactory.isLteDcSupport()){            
            if (mPsArbitrator == null && !(this instanceof LteDcManager)){    
                riljLog("[RilPsArbitrator] initialize the PsArbitrator!");  
                mPsArbitratorThread = new HandlerThread("PsArbitrator");
                mPsArbitratorThread.start();
                
                Looper looper = mPsArbitratorThread.getLooper();
                mPsArbitrator = new PsArbitrator(looper, RIL.this/*, RIL.this.mPhone*/);
                //mPsArbitrator = new PsArbitrator(looper, RIL.this, RIL.this.mPhone);
            }     
        }

        if (LteModemSwitchHandler.getActiveModemType() != LteModemSwitchHandler.MD_TYPE_LTNG) return null;
            
        return mPsArbitrator;
    }
    //[MMDC] PSDM end
...
//Add by gfzhu VIA end
 ...
 /// M:For oplmn update feature. @{
    ...
    // / @}
    // overwrite
    public LteDcManager getLteDcManager() {
        if((this instanceof LteDcManager)) {
            if (RILJ_LOGD) riljLog("should not get LteDcManager from LteDcManager mySimId:" + mySimId);
            return null;
        }

        if (RILJ_LOGD) riljLog("getLteDcManager, mySimId: " + mySimId);
        if(mLteDcManager == null) {
            if (RILJ_LOGD) riljLog("mLteDcManager is null");
        }
        return mLteDcManager;
    }
