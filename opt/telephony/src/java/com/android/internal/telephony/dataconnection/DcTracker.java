...
public class DcTracker extends DcTrackerBase {
    ...
    protected void notifyOffApnsOfAvailability(String reason) {
        for (ApnContext apnContext : mApnContexts.values()) {
            if ((!mAttached.get() && FeatureOption.MTK_GEMINI_SUPPORT) || !apnContext.isReady()) {//[KK]
            //if (!apnContext.isReady()) {             
   ...
   //[mr2] Create ApnList end
    ...
    private void onApnChanged() {
        ...
        if ((previous_allApns == null || previous_allApns.isEmpty()) &&
        ...
        } else if (!(previous_allApns == null || previous_allApns.isEmpty()) &&
                !(mAllApnSettings == null || mAllApnSettings.isEmpty()))
        {
            String pre_all_str = "";
            String all_str = "";
            ...
            if (pre_all_str.equals(all_str)) {
                if (previous_preferredApn == null && mPreferredApn == null) {
                    ...
                    isSameApnSetting = true;
                    //TODO MTK remove
                } else if (previous_preferredApn == null && mPreferredApn != null) {
                    //no need to cleauup all and re-activate
                    //since the preferred is set only if default apn is activated
                    isSameApnSetting = true;
                }
            } else { 
        ...
        if (imsi != null && imsi.length() > 0) {
            if (!FeatureOption.MTK_GEMINI_SUPPORT) {
                ...
                setInitialAttachApn();
                if (DBG) log("onApnChanged: previousAttachApn [" + previousAttachApn + "] mInitialAttachApnSetting [" + mInitialAttachApnSetting + "]");
            }
        } else {
            if (DBG) log("onApnChanged: but no imsi");
        }

        if (mInitialAttachApnSetting != null && previousAttachApn != null) {
            if (!previousAttachApn.toString().equals(mInitialAttachApnSetting.toString())) {
                isSameApnSetting = false;
            }
        } else if (mInitialAttachApnSetting == null && previousAttachApn == null) {
            //do nothing since attach apn is not changed
        } else {
            isSameApnSetting = false;
        }
    ...
    */
    ...
    private void onRecordsLoaded() {
        ...
        if(PhoneFactory.isLteDcSupport()){
           mPhone.setPsConnType(GeminiNetworkSubUtil.CONN_TYPE_ALWAYS);
        } else if (gprsDefaultSIM == nSlotId) {
    ...
    private void applyNewState(ApnContext apnContext, boolean enabled, boolean met) {
        ...
        //[MMDC]
        if (FeatureOption.MTK_LTE_DC_SUPPORT && LteModemSwitchHandler.getActiveModemType() == LteModemSwitchHandler.MD_TYPE_LTNG) {
            String[] activeApnTypes = getActiveApnTypes();
            if ((enabled == false && (activeApnTypes == null || activeApnTypes.length == 0)) ||
                    (enabled == true && (activeApnTypes != null && activeApnTypes.length == 1))) {
                // Only notify change as no active APNs or the first APN enabled
                log("applyNewState(): notify data usage change.");
                
                mGsmPhone.getServiceStateTracker().notifyDataUsageChange();                
            }
        }
    ...
    protected void onDataSetupComplete(AsyncResult ar) {
        ...
        if (ar.exception == null) {
            ...
            if (cause != DcFailCause.RADIO_NOT_AVAILABLE && cause != DcFailCause.RADIO_POWER_OFF) {
                //if the error is caused by radio not available or power off
                //we should not remove waiting apn since we expect use the same apn
                //when radio is available again
    ...
    protected void onDisconnectDone(int connId, AsyncResult ar) {
        ...
        if (isDisconnected()) {
            ...
            if (FeatureOption.MTK_LTE_DC_SUPPORT && LteModemSwitchHandler.getActiveModemType() == LteModemSwitchHandler.MD_TYPE_LTNG) {
                ...
                log("onDisconnectDone(): notify data usage change.");
                if(!isAnyApnCtxEnabled()){
                    log("[PsArbitrator]if data off before, now use data will reset decision");
                    //mPhone.mCi.getPsArbitrator().clearSettings();
                }                    
                mGsmPhone.getServiceStateTracker().notifyDataUsageChange();                
        ...
        if ((mAttached.get() || !FeatureOption.MTK_GEMINI_SUPPORT) && apnContext.isReady() && retryAfterDisconnected(apnContext)) {
            SystemProperties.set(PUPPET_MASTER_RADIO_STRESS_TEST, "false");
            // Wait a bit before trying the next APN, so that
            // we're not tying up the RIL command channel.
        ...
        if (usePreferred && mCanSetPreferApn && mPreferredApn != null &&
            ...
            if (mPreferredApn.numeric.equals(operator)) {
                if (mPreferredApn.bearer == 0 || mPreferredApn.bearer == radioTech) {
                    ...
                    return apnList;
                } else if (mPreferredApn.bearer != 0 && radioTech == 0) {
                    //in this case we choose not to build waiting apn
                    //since we should trysetup data again after data attached
                    if (DBG) log("buildWaitingApns: radio tech is unknown but preferred bearer is " + mPreferredApn.bearer);
                    return apnList;
                } else {
                    //Sometimes current radio technology is different then bearer settings
                    //of the preferred apn. Not to change preferred apn in this scenario
                    //to prevent users being confused
                    if (DBG) {
                        log("buildWaitingApns: and preferred APN bearer is different" +
                                " than current radio technology");
                    }
    ...
    protected void sendScriCmd(boolean retry) {
        try{
            int enableFdOnCharging = Integer.parseInt(SystemProperties.get("fd.on.charge", "0"));            
            
            if((mScriManager.getPsSessionStatus() || retry) && 
               (mPhone.getState() == PhoneConstants.State.IDLE) && 
               mIsUmtsMode && 
               (!mIsTetheredMode) && (!mChargingMode || (enableFdOnCharging == 1)))   //[ALPS00098656][mtk04070]Disable Fast Dormancy when in Tethered mode
            {
            ...
            if (action.equals(TelephonyIntents.ACTION_SIM_STATE_CHANGED)){  //add by MTK03594 for SCRI
                if (!FeatureOption.MTK_GEMINI_SUPPORT) {
                    ...
                    if (IccCardConstants.INTENT_VALUE_ICC_IMSI.equals(state)) {
                        String imsi = mPhone.getSubscriberId();
                        logd("IA: Receive ACTION_SIM_STATE_CHANGED state=" + state + ", oldImsi=" + mImsi + ", imsi=" + imsi);
                        if (mImsi != null && imsi != null && !mImsi.equals(imsi)) {
                            //for SIM refresh case, the attach apn would be changed
                            //need to set initial attach apn again
                            if (!FeatureOption.MTK_GEMINI_SUPPORT) {
                                //currently only single SIM solution support this
                                setInitialAttachApn();
                            }
    ...
    // M: For MMDC support to check is any Apn context is enabled
    @Override
    public boolean isAnyApnCtxEnabled() {
        boolean retValue = false;

        mUserDataEnabled = Settings.Global.getInt(
            mPhone.getContext().getContentResolver(), Settings.Global.MOBILE_DATA, 1) == 1;
        SystemProperties.set("ril.mobile.data", (mUserDataEnabled ? "1" : "0"));
        if (mUserDataEnabled) {
            if (DBG) logd("isAnyApnCtxEnabled(): Default data is on.");
            return true;
        }
        for (ApnContext apnContext : mApnContexts.values()) {
            if (apnContext.isReady() && apnContext.getApnType() != PhoneConstants.APN_TYPE_DEFAULT) {
                if (DBG) logd("isAnyApnCtxEnabled(): APN " + apnContext.getApnType() + " is enabled.");
                retValue = true;
                break;
            }
        }
