...
#define VIDEO_POOL_MEMORY 1000000
    ...
    ALOGD("M4MP4W: %d images max", nbVideoFrames);
...
/*******************************************************************************/
{
    ...
    ALOGD("tempFileDescriptor = %p", tempFileDescriptor);
...
/*******************************************************************************/
{
        ...
        && M4OSA_NULL != mMp4FileDataPtr->safetyFileUrl)
    {
        ...
        ALOGD("Safety file correctly created");
    }
#endif /* _M4MP4W_RESERVED_MOOV_DISK_SPACE */
...
/*******************************************************************************/
{
    ...
    ALOGD(" M4MP4W_newAudioChunk - flush audio");
    ALOGD("current chunk = %d  offset = 0x%x",
#else
    ALOGD(" M4MP4W_newAudioChunk - flush audio");
    ALOGD("current chunk = %d  offset = 0x%x",
#endif
            ...
            > mMp4FileDataPtr->audioTrackPtr->LastAllocatedChunk)
    {
        ...
        {
            ALOGD("M4MP4W_newAudioChunk : audio chunk table is full");
            return M4WAR_MP4W_OVERSIZE;
        }
    ...
    *leftSpaceInChunk = mMp4FileDataPtr->audioTrackPtr->MaxChunkSize;
    ...
    if (mMp4FileDataPtr->audioTrackPtr->currentStsc
        + 3 > M4MP4W_CHUNK_AUDIO_ALLOC_NB)
    {
        ALOGD("M4MP4W_newAudioChunk : audio stsc table is full");
        return M4WAR_MP4W_OVERSIZE;
    }
    ...
    //ALOGD("CommonData.lastCTS =%x, audioTrackPtr->currentStsc=%x ",
    //mMp4FileDataPtr->audioTrackPtr->CommonData.lastCTS,mMp4FileDataPtr->audioTrackPtr->currentStsc);
...
/*******************************************************************************/
{
    ...
    ALOGD("M4MP4W_newVideoChunk - flush video");
    ALOGD("current chunk = %d  offset = 0x%x",
        mMp4FileDataPtr->videoTrackPtr->currentChunk,
        mMp4FileDataPtr->absoluteCurrentPos);
        ...
        > mMp4FileDataPtr->videoTrackPtr->LastAllocatedChunk)
    {
        ...
        {
            ALOGD("M4MP4W_newVideoChunk : video chunk table is full");
            return M4WAR_MP4W_OVERSIZE;
        }
    ...
    *leftSpaceInChunk = mMp4FileDataPtr->videoTrackPtr->MaxChunkSize;
    ...
    if (mMp4FileDataPtr->videoTrackPtr->currentStsc
        + 3 > M4MP4W_CHUNK_ALLOC_NB)
    {
        ALOGD("M4MP4W_newVideoChunk : video stsc table is full");
        return M4WAR_MP4W_OVERSIZE;
    }
...
/*******************************************************************************/
{
    ...
    else if (streamID == VideoStreamID) /*video stream*/
    {
        ...
        ALOGD("startAU: size 0x%x pos 0x%x chunk %u", auPtr->size,
            mMp4FileDataPtr->videoTrackPtr->currentPos,
            mMp4FileDataPtr->videoTrackPtr->currentChunk);

        ALOGD("adr = 0x%p", auPtr->dataAddress);

        if (auPtr->dataAddress)
        {
            ALOGD(" data = %08X %08X %08X", auPtr->dataAddress[0],
...
#ifndef ANDROID_DEFAULT_CODE
    ALOGD("M4MPW_startAU: start address:%p, size:%lu, stream:%d",
#else
    ALOGD("M4MPW_startAU: start address:%p, size:%lu, stream:%d",
#endif
...
/*******************************************************************************/
{
        ...
        || (streamID == VideoStreamID))
    {
        ...
        if (mMp4FileDataPtr->MaxFileSize != 0)
        {
            ...
            {
                ALOGD("processAU : !! FILESIZE EXCEEDED !!");

                /* patch for autostop is MaxFileSize exceeded */
                ALOGD("M4MP4W_processAU : stop at targeted filesize");
                return M4WAR_MP4W_OVERSIZE;
            }
        }
    }
        ...
        && (streamID == AudioStreamID))
    {
        if (mMp4FileDataPtr->audioMsStopTime <= (auPtr->CTS *scale_audio))
        {
            ...
            ALOGD("M4MP4W_processAU : audio stop time reached");
            return M4WAR_MP4W_OVERSIZE;
        }
    }
    ...
    if (streamID == AudioStreamID) /*audio stream*/
    {
        ...
        else
        {
            ...
            {
                ALOGD("M4MP4W_processAU : audio stts table is full");
                return M4WAR_MP4W_OVERSIZE;
            }
    ...
    else if (streamID == VideoStreamID) /*video stream*/
    {
        ...
        if ((M4MP4W_Time32)auPtr->CTS < mMp4FileDataPtr->videoTrackPtr->CommonData.lastCTS) {
            ...
            ALOGD("Skip frame. Video frame has too old timestamp.");
            return M4NO_ERROR;
        }
        ...
        {
            ALOGD("M4MP4W_processAU : video stsz table is full");
            return M4WAR_MP4W_OVERSIZE;
        }
        ...
        else
        {
            ...
            {
                ALOGD("M4MP4W_processAU : video stts table is full");
                return M4WAR_MP4W_OVERSIZE;
            }
        ...
        if (auPtr->attribute == AU_RAP)
        {
            ...
            {
                ALOGD("M4MP4W_processAU : video stss table is full");
                return M4WAR_MP4W_OVERSIZE;
            }
...
#ifndef ANDROID_DEFAULT_CODE
    ALOGD("processAU : size 0x%x mode %d filesize %lu limit %lu",
#else
    ALOGD("processAU : size 0x%x mode %d filesize %lu limit %lu",
...
/*******************************************************************************/
{
    ...
    if (bVideo)
    {
...
#ifndef ANDROID_DEFAULT_CODE
        ALOGD("video | CLOSE: current chunk = %d  offset = 0x%x size = 0x%08X",
#else
        ALOGD("current chunk = %d  offset = 0x%x size = 0x%08X",
#endif //ANDROID_DEFAULT_CODE
            mMp4FileDataPtr->videoTrackPtr->currentChunk,
            mMp4FileDataPtr->absoluteCurrentPos,
            mMp4FileDataPtr->videoTrackPtr->currentPos);
...
#ifndef ANDROID_DEFAULT_CODE
        ALOGD("MaxFileDuration = %d", mMp4FileDataPtr->MaxFileDuration);
#endif //ANDROID_DEFAULT_CODE
        if (mMp4FileDataPtr->MaxFileDuration > 0)
        {
            ...
            if (mMp4FileDataPtr->videoTrackPtr->CommonData.sampleNb > 1)
            {
                ...
                if (8 *mMp4FileDataPtr->videoTrackPtr->CommonData.sttsTableEntryNb
                    ...
                    *M4MP4W_STTS_ALLOC_SIZE)
                {
                    ALOGD("sttsTableEntryNb %d, nbOfAllocatedSttsBlocks %d", 
                        mMp4FileDataPtr->videoTrackPtr->CommonData.sttsTableEntryNb,
                        mMp4FileDataPtr->videoTrackPtr->nbOfAllocatedSttsBlocks);
                    mMp4FileDataPtr->videoTrackPtr->nbOfAllocatedSttsBlocks += 1;
                    mMp4FileDataPtr->videoTrackPtr->TABLE_STTS =
                    ...
                    {
                        ALOGD("M4MP4W_realloc new TABLE_STTS failed");
                        mMp4FileDataPtr->fileWriterFunctions->closeWrite(
                        fileWriterContext); /**< close the stream anyway */
                        M4MP4W_freeContext(context); /**< Free the context content */
                        return M4ERR_ALLOC;
...
#ifndef ANDROID_DEFAULT_CODE
            ALOGD("MaxFileDuration = %d  delta = %d sttsTableEntryNb = %d, nbOfAllocatedSttsBlocks %d",
        ...
        else
        {
            ...
            {
#ifndef ANDROID_DEFAULT_CODE
                ALOGD("sampleNb = %d, sttsTableEntryNb = %d, nbOfAllocatedSttsBlocks %d",
                            mMp4FileDataPtr->videoTrackPtr->CommonData.sampleNb,
                            mMp4FileDataPtr->videoTrackPtr->CommonData.sttsTableEntryNb,
                            mMp4FileDataPtr->videoTrackPtr->nbOfAllocatedSttsBlocks);
#endif //ANDROID_DEFAULT_CODE
            ...
            {
                ALOGD("M4MP4W_closeWrite : ! videoTrackPtr,\
    ...
    if (bVideo)
    {
        ...
        if (bH264)
        {
            ...
            {
                ALOGD("!!! M4MP4W_closeWrite ERROR : invalid AVCC 0x%X 0x%X",
...
cleanup:
    ...
    * M4MP4W_freeContext() is now a private method, called only from here*/
    ...
    if (M4NO_ERROR != err3)
    {
        ALOGD("M4MP4W_closeWrite: M4MP4W_freeContext returns 0x%x",
            err3);
    }
    ...
    * Choose which error code to return */
    if (M4NO_ERROR != err)
    {
        ...
        * We give priority to main error */
        ALOGD("M4MP4W_closeWrite: returning err=0x%x", err);
        return err;
    }
    else if (M4NO_ERROR != err2)
    {
        ...
        * Error from closeWrite is returned if there is no main error */
        ALOGD("M4MP4W_closeWrite: returning err2=0x%x", err2);
        return err2;
    }
    else
    {
        ...
        * Error from M4MP4W_freeContext is returned only if there is no main error and
          no close error */
        ALOGD("M4MP4W_closeWrite: returning err3=0x%x", err3);
        return err3;
    }
}
