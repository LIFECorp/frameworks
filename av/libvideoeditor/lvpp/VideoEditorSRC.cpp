...
status_t VideoEditorSRC::read(
    ...
    *buffer_out = NULL;
	...
	ALOGD("mFormatChanged=%d", mFormatChanged);
        ...
        if (mFormatChanged) {
            ...
            {
                 free(pTmpBuffer);
		 ALOGD("VideoEditorSRC::read ERROR_FORBIDDEN");				 
                 return ERROR_FORBIDDEN;
            }
#else			
            checkAndSetResampler();
...
void VideoEditorSRC::checkAndSetResampler() {
    ...
    ALOGD(" checkAndSetResampler: mChannelCnt =%d",mChannelCnt);
    if (mChannelCnt > 2)
    {
	ALOGD(" checkAndSetResampler: ERROR_FORBIDDEN");
