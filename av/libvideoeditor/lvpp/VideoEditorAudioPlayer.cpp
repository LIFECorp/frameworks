...
status_t VideoEditorAudioPlayer::start(bool sourceAlreadyStarted) {
    ...
    CHECK(!mStarted);
    CHECK(mSource != NULL);
    ALOGV("Start %d",sourceAlreadyStarted);
    status_t err;
    ...
    if (mFirstBufferResult == ERROR_FORBIDDEN) {
	return ERROR_FORBIDDEN;
    }
    else if (mFirstBufferResult == INFO_FORMAT_CHANGED) {
#else	
    ...
    ALOGD(" VideoEditorAudioPlayer::start=%d",numChannels);
    if (numChannels > 2)
    {
	ALOGD(" VideoEditorAudioPlayer::start: ERROR_FORBIDDEN");
