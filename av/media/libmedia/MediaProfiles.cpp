...
MediaProfiles::createDefaultHEVCVideoEncoderCap()
{
	...
	switch(eChipName)
	{
        ...
        {
            return new MediaProfiles::VideoEncoderCap(
                VIDEO_ENCODER_HEVC, /*250000*/75000, 60000*1000, 128,
                3840, 128, 2176, 15, 30);
        }
    ...
    switch (i4VideoFormat)
    {
        ...
        case VIDEO_ENCODER_HEVC :
            ...
            if(ret ==  VENC_DRV_MRESULT_OK){
                ...
                ALOGI("[VIDEO_ENCODER_HEVC] checkVideoCapability, format=%d,support maxwidth=%d,maxheight=%d, bitrate %d, framerate %d",i4VideoFormat,outinfo.u4Width,outinfo.u4Height, outinfo.u4Bitrate, outinfo.u4FrameRate);
            }
            else{
                i4RetValue = -1;
            }
...
#if !defined(ANDROID_DEFAULT_CODE)
static unsigned long getChipVariant(void)
{
    ...
    return (unsigned long)outChipVariant;
}
#endif
...
MediaProfiles::createMTKCamcorderProfile(camcorder_quality quality, camcorder_mode CamMode, camera_id CamId)
{
    ...
    // Get Chip Variant
    unsigned long eChipVariant = 0xFFFFFFFF;
			...
			switch(eChipName)
			{
                ...
                case VAL_CHIP_NAME_MT6577:
				{
					if (( (VAL_CHIP_NAME_MT6571 == eChipName) && (VAL_CHIP_VARIANT_MT6571L == eChipVariant)) ||
                                                                                    ( (VAL_CHIP_NAME_MT6575 == eChipName)))
					{
						videoCodec = new MediaProfiles::VideoCodec(VIDEO_ENCODER_MPEG_4_SP, 4000*1000/CamMode, 480, 320, 30/CamMode);
					}
					else
			...
			switch(eChipName)
			{
                ...
                case VAL_CHIP_NAME_MT6577:
                                                                                     ...
                                                                                     ((VAL_CHIP_NAME_MT6575 == eChipName)))
					{
						...
						if (CamId == BACK_CAMERA){
							videoCodec = new MediaProfiles::VideoCodec(VIDEO_ENCODER_MPEG_4_SP, 6000*1000*i4FrameRate/30, 640, 480, i4FrameRate);
						}
						else{
							videoCodec = new MediaProfiles::VideoCodec(VIDEO_ENCODER_MPEG_4_SP, 4200*1000*i4FrameRate/30, 640, 480, i4FrameRate);
						}
					}
					else
					{
		...
		case CAMCORDER_QUALITY_MTK_TIME_LAPSE_NIGHT_FINE:
			switch(eChipName)
			{
                ...
                case VAL_CHIP_NAME_MT6577:
				{
					...
					{
						// 71L
						videoCodec = new MediaProfiles::VideoCodec(VIDEO_ENCODER_MPEG_4_SP, 4000*1000, 864, 480, 15);
					}
					else
					{
						if(getVideoCapability(VIDEO_ENCODER_MPEG_4_SP, &u4Width, &u4Height, &u4BitRate, &u4FrameRate ) > 0)
                ...
                case VAL_CHIP_NAME_ROME:
                    {
                        videoCodec = new MediaProfiles::VideoCodec(VIDEO_ENCODER_HEVC, 60000*1000/CamMode, 3840, 2176, 30/CamMode);
                    }
                    break;
#endif
				#endif
