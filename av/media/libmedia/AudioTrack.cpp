...
#ifdef MTB_SUPPORT
	ATRACE_ONESHOT(ATRACE_ONESHOT_AUDIOTRACK, "AudTrk::set, %d, %d, %d, %d, %u, %04x", streamType, sampleRate, format,channelMask,frameCount, flags); 
#endif
#endif
...
status_t AudioTrack::start()
{
    AutoMutex lock(mLock);
#ifdef MTB_SUPPORT
	ATRACE_ONESHOT(ATRACE_ONESHOT_AUDIOTRACK, "AudTrk::start"); 
#endif
...
void AudioTrack::stop()
{
    AutoMutex lock(mLock);
#ifdef MTB_SUPPORT
	ATRACE_ONESHOT(ATRACE_ONESHOT_AUDIOTRACK, "AudTrk::stop"); 
#endif
...
{
#ifdef MTB_SUPPORT
	ATRACE_ONESHOT(ATRACE_ONESHOT_AUDIOTRACK, "AudTrk::flush"); 
#endif
    if (mSharedBuffer != 0) {
        return;
    }
...
#endif
#ifdef MTB_SUPPORT
	ATRACE_ONESHOT(ATRACE_ONESHOT_AUDIOTRACK, "AudTrk::pause"); 
#endif
    AutoMutex lock(mLock);
    if (mState == STATE_ACTIVE) {
        mState = STATE_PAUSED;
...
{
#ifdef MTB_SUPPORT
	ATRACE_ONESHOT(ATRACE_ONESHOT_AUDIOTRACK, "AudTrk::Write, tid: %d", (unsigned int)gettid()); 
#endif
