...
#ifndef MTK_IPTV_SUPPORT
                msg->post(100000ll);
#else
                msg->post(1000ll);  // reduce scan  source time
#endif
...
void NuPlayer::finishConsumeIfPossible() {
    ...
    if (!mAudioAbsent && (mConsumingAudio != Consume_DONE && mConsumingAudio != Consume_NONE)) {
        return;
    }
    if (!mVideoAbsent && (mConsumingVideo != Consume_DONE && mConsumingVideo != Consume_NONE)) {
        return;
    }
...
#endif
#ifdef MTK_POST_PROCESS_FRAMEWORK_SUPPORT
        // format->setInt32("use-postprocessing-mode", mEnPostProcessing);
#endif
...
#ifdef MTB_SUPPORT
	if(audio)
		ATRACE_ONESHOT(ATRACE_ONESHOT_SPECIAL, "omxAquireAudioBuffer");	 
	else
		ATRACE_ONESHOT(ATRACE_ONESHOT_SPECIAL, "omxAquireVideoBuffer");	 
#endif
...
#ifdef MTB_SUPPORT
	if(audio)
		ATRACE_ONESHOT(ATRACE_ONESHOT_SPECIAL, "feedOneAudioBuffer");	 
	else
		ATRACE_ONESHOT(ATRACE_ONESHOT_SPECIAL, "feedOneVideoBuffer");	 
#endif
...
#ifdef MTB_SUPPORT
	if(audio)
		ATRACE_ONESHOT(ATRACE_ONESHOT_SPECIAL, "queueOneAudioBuffer");   
	else
		ATRACE_ONESHOT(ATRACE_ONESHOT_SPECIAL, "queueOneVideoBuffer");   
#endif
