...
#if APPLY_CHECKING_FBDEOS_COMPLETED
      mTotalTimeDuringCheckFBDEOS(0),
      mPortsFBDEOSComplete(0),
...
#endif //APPLY_CHECKING_FLUSH_COMPLETED
#if APPLY_CHECKING_FBDEOS_COMPLETED
void ACodec::signalVDecFBDEOSDoneCheck(int delayTime, int mTimeOut) {
    sp<AMessage> msg = new AMessage(kWhatMtkVdecCheckFBDEOSDone, id());
    msg->setInt32("MtkVDecFBDEOSDoneCheckDelayTime", delayTime);
    msg->setInt32("MtkVDecFBDEOSDoneCheckTimeOut", mTimeOut);
    msg->post(delayTime);
    ALOGD("signalVDecFBDEOSDoneCheck");
}
#endif //APPLY_CHECKING_FBDEOS_COMPLETED
#endif//ANDROID_DEFAULT_CODE
...
#endif  // USE_VIDEO_ION
            //for CTS android.media.cts.EncodeVirtualDisplayWithCompositionTest, reduce these buffer size
            if( (portIndex == kPortIndexInput) && (mIsVideoEncoder) && (mIsVideoEncoderInputSurface==1) )
            {
                def.nBufferSize = MTK_BUF_ADDR_ALIGNMENT_VALUE;
                ALOGD("Encoder using inputSurface allocate %d only", def.nBufferSize);
            }
#endif  // ANDROID_DEFAULT_CODE
...
#if  ((defined MTK_CLEARMOTION_SUPPORT) || (defined MTK_POST_PROCESS_FRAMEWORK_SUPPORT))
    ...
    if (video) {
        if (encoder) {
            ...
            if(mACodecColorConverter)
            {
                ...
                mACodecColorConverter->mMtkColorConvertInfo->mDstFormat = eYUV_420_3P;//android output default is YV12
#else
...
#ifndef ANDROID_DEFAULT_CODE
#ifdef MTK_IPTV_SUPPORT
    else if (!strcasecmp(mime, MEDIA_MIMETYPE_AUDIO_MPEG)) {
        ALOGV("set MPEG audio config");
        int32_t numChannels, sampleRate;
        if (encoder
                || !msg->findInt32("channel-count", &numChannels)
                || !msg->findInt32("sample-rate", &sampleRate)) {
            ALOGE("set MPEG config INVALID");
            err = INVALID_OPERATION;
        } else {
            ALOGD("set MPEG config. %d, %d", numChannels, sampleRate);
            ...
            profileMp3.nChannels=numChannels;
            profileMp3.nSampleRate=sampleRate;     
            err = mOMX->setParameter(
                mNode, OMX_IndexParamAudioMp3, &profileMp3, sizeof(profileMp3));  
            CHECK_EQ((status_t)OK, err);
...
#ifdef MTB_SUPPORT
        ATRACE_BEGIN_EXT("Acode_NW_QB");
#endif
        err = mNativeWindow->queueBuffer(mNativeWindow.get(),
                buf->getNativeBuffer(), -1);
#ifdef MTB_SUPPORT
        ATRACE_END_EXT("Acode_NW_QB");
#endif
        if (err != NO_ERROR) {
...
status_t ACodec::setVDecCheckFlushDone(int delayTime, int mTimeOut) {
    ...
    if (!strncmp(mComponentName.c_str(), "OMX.MTK.VIDEO.DECODER", 21)) {
        ALOGI("check Flushing, mInputEOSResult %d, %d, %d, %d", mInputEOSResult, delayTime, mTotalTimeDuringCheckFlush, mTimeOut);

        if( (mTotalTimeDuringCheckFlush+delayTime) >= mTimeOut )
        {
            ALOGW("Timeout and log call stack after flushing %d, > %d us", mTotalTimeDuringCheckFlush, mTimeOut);
...
#endif //APPLY_CHECKING_FLUSH_COMPLETED
#if APPLY_CHECKING_FBDEOS_COMPLETED
status_t ACodec::setVDecCheckFBDEOSDone(int delayTime, int mTimeOut) {
    if (mIsEncoder) {
        return ERROR_UNSUPPORTED;
    }
    if (!strncmp(mComponentName.c_str(), "OMX.MTK.VIDEO.DECODER", 21)) {
        ALOGI("check FBDEOS, mInputEOSResult %d, %d, %d, %d", mInputEOSResult, delayTime, mTotalTimeDuringCheckFBDEOS, mTimeOut);

        if( (mTotalTimeDuringCheckFBDEOS+delayTime) >= mTimeOut )
        {
            ALOGW("Timeout and log call stack after FBDEOS %d, > %d us", mTotalTimeDuringCheckFBDEOS, mTimeOut);
            android::CallStack stack(LOG_TAG);
            //trigger NE
            CHECK_EQ(mTotalTimeDuringCheckFBDEOS, mTimeOut);
        }
        #if 1//first seek with mInputEOSResult = OK
        else if( (mTotalTimeDuringCheckFBDEOS > 0) && (mPortsFBDEOSComplete == 1) )
        {
            ALOGD("FBDEOS done %d, stop checking", mTotalTimeDuringCheckFBDEOS);
        }
        #endif
        else
        {
            ...
            msg->setInt32("MtkVDecFBDEOSDoneCheckTimeOut", mTimeOut);
            msg->post(delayTime);
            mTotalTimeDuringCheckFBDEOS += delayTime;
            ALOGD("signalVDecFBDEOSDoneCheck again after %d us", mTotalTimeDuringCheckFBDEOS);
        }
    }
    return OK;
}
status_t ACodec::signalVDecCheckFBSEOSDone(){

    if ( (!mIsEncoder) && (!strncmp(mComponentName.c_str(), "OMX.MTK.VIDEO.DECODER", 21)) )
    {
        int32_t dummy = 0;
        char value[PROPERTY_VALUE_MAX];
        property_get("acodec.video.checkFBDEOSTimeOut", value, "0");
        dummy = atof(value);
        if (dummy > 0) {
            signalVDecFBDEOSDoneCheck(50000, dummy);//50ms, time out from user Setting
            ALOGD ("acodec.video.checkFBDEOSTimeOut %d us", dummy);
        }
        else
            signalVDecFBDEOSDoneCheck(50000, 8000000);//50ms, 8000ms time out for flushing ports
    }
...
#endif //APPLY_CHECKING_FLUSH_COMPLETED
#if APPLY_CHECKING_FBDEOS_COMPLETED
        case kWhatMtkVdecCheckFBDEOSDone:
        {
            int delayTime;
            int mTimeOut;
            msg->findInt32("MtkVDecFBDEOSDoneCheckDelayTime", &delayTime);
            msg->findInt32("MtkVDecFBDEOSDoneCheckTimeOut", &mTimeOut);
            status_t err = mCodec->setVDecCheckFBDEOSDone(delayTime, mTimeOut);
            if( OK != err )
                ALOGD("Acodec check Vdec(should be) FBDEOS done ret: %x", err);
            break;
        }
#endif //APPLY_CHECKING_FBDEOS_COMPLETED
#endif//ANDROID_DEFAULT_CODE
...
#ifndef ANDROID_DEFAULT_CODE
    ALOGV("ACodec::BaseState::postFillThisBuffer mPortEOS %d", mCodec->mPortEOS[kPortIndexInput]);
#endif //ANDROID_DEFAULT_CODE
...
#ifndef ANDROID_DEFAULT_CODE
    ALOGV("onInputBufferFilled bufferID %x", bufferID);
#endif //ANDROID_DEFAULT_CODE
    sp<ABuffer> buffer;
    int32_t err = OK;
    bool eos = false;
...
#ifndef ANDROID_DEFAULT_CODE
#if APPLY_CHECKING_FBDEOS_COMPLETED
                    mCodec->signalVDecCheckFBSEOSDone();
#endif //APPLY_CHECKING_FBDEOS_COMPLETED
#endif //ANDROID_DEFAULT_CODE
                }
...
bool ACodec::BaseState::onOMXFillBufferDone(
    ...
    if( (!strncmp("OMX.MTK.VIDEO.DECODER.", mCodec->mComponentName.c_str(), strlen("OMX.MTK.VIDEO.DECODER."))) && (mCodec->mIsDemandNormalYUV==true))
    {
        ...
        //if (!strncmp("OMX.MTK.", mComponentName.c_str(), 8)) 
        {
            ...
            ALOGV ("mSrcFormat(%x), eHalColorFormat(%x)", mSrcFormat, eHalColorFormat);
        }
        ...
        if ( (true == mCodec->mIsDumpRawFile) && (info->mData->data() != NULL)) {
...
#ifndef ANDROID_DEFAULT_CODE
#if APPLY_CHECKING_FBDEOS_COMPLETED
                if( 0 == mCodec->mPortsFBDEOSComplete )
                {
                    ALOGD("FBDEOS Completed after signal FBDEOS %d ms", mCodec->mTotalTimeDuringCheckFBDEOS);
                    mCodec->mTotalTimeDuringCheckFBDEOS++;
                    mCodec->mPortsFBDEOSComplete = 1;
                }
#endif //APPLY_CHECKING_FBDEOS_COMPLETED
#endif //ANDROID_DEFAULT_CODE
            }
...
int ACodec::BaseState::Mtk_ACodecConvertDecoderOutputToI420(
    ...
    else
    {
        ...
        if ( (true == mCodec->mIsDumpProflingFile) && (diff != 0) ) {
            //char tmpW[2] = "\n";
            //if dump MTK blk yuv, size: YUV_SIZE(ROUND_16(videoDef->nFrameWidth), ROUND_32(videoDef->nFrameHeight))
            if ((!strncmp(mCodec->mComponentName.c_str(), "OMX.MTK.VIDEO.DECODER.", strlen("OMX.MTK.VIDEO.DECODER.")))) {
                ...
                sprintf(ucStringyuv, "//sdcard/Profiling%4d.txt",  gettid());
...
#ifndef ANDROID_DEFAULT_CODE
#if APPLY_CHECKING_FLUSH_COMPLETED
            if ( (!mCodec->mIsEncoder) && (!strncmp(mCodec->mComponentName.c_str(), "OMX.MTK.VIDEO.DECODER", 21)) )
            {
                int32_t dummy = 0;
                char value[PROPERTY_VALUE_MAX];
...
#ifndef ANDROID_DEFAULT_CODE
#if APPLY_CHECKING_FLUSH_COMPLETED
        ALOGD("send kWhatFlushCompleted after signal flush %d ms", mCodec->mTotalTimeDuringCheckFlush);
