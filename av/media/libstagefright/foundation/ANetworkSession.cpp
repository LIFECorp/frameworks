...
status_t ANetworkSession::Session::readMore() {
    if (mState == DATAGRAM) {
            ...
            if (n < 0) {
				...
				else
				{
					if (fNextExpectedSeqNo != fSeqNo)
					{
						...
						if(diff >60000)
						{
							//ALOGD("Wrong order, Recv SeqNo = %u, Expect SeqNo = %u\n",
							//	fSeqNo, fNextExpectedSeqNo);
							continue;						
						}

						//ALOGD("Recv SeqNo = %u, Expect SeqNo = %u, Packet lost = %d\n",
        ...
        } while (err == OK && fReadCount < MAX_READ_COUNT);
