...
void PlaylistFetcher::onDownloadNext() {
...
#ifndef ANDROID_DEFAULT_CODE
            } else if (errcode == -ENOTCONN) {
                ALOGE("failed to load playlist at url '%s' cuz seek's disconnection", mURI.c_str());
                return;
#endif
            } else {
    ...
    if (err != OK) {
         ...
         // mtk80902: ALPS01428240 - side effect of disconnect when seeking
         if(err == -ENOTCONN) {
...
#ifndef ANDROID_DEFAULT_CODE
            else if((packetSource->getPendingFormat() == NULL) && (packetSource->getFormat().get() != source->getFormat().get())) //new stream
