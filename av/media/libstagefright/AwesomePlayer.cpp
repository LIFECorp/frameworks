...
#ifdef MTB_SUPPORT
    ATRACE_ONESHOT(ATRACE_ONESHOT_EVENT, "_onStreamDone");
#endif
...
status_t AwesomePlayer::pause_l(bool at_eos) {
	...
	if (mVideoRenderingStarted && mMJCPauseDelay && !(mFlags & VIDEO_AT_EOS)) {
...
#ifdef MTB_SUPPORT
    ATRACE_ONESHOT(ATRACE_ONESHOT_EVENT, "_onVideoEvent");
#endif
...
#ifdef MTB_SUPPORT
                ATRACE_BEGIN_EXT("read VB");
#endif                
                err = mVideoSource->read(&mVideoBuffer, &options);
#ifdef MTB_SUPPORT                
                ATRACE_END_EXT("read VB");
#endif                
            }
#else
...
//wait for 			
	...
	if((mSMSpeed == 1)||(mSMSpeed == 2)||(mSMSpeed == 4)){
		if(mSpeedValue != mSMSpeed){
			...
			ALOGD("mSpeedValue changed from %d to %d",mSpeedValue,mSMSpeed);			
			SpeedValueChanged = true;
			if(mAudioSink != NULL){
				if(mSpeedValue == 1)
					mAudioSink->setParameters(String8("time_stretch=1"));
				else if(mSpeedValue == 2)
					mAudioSink->setParameters(String8("time_stretch=2"));
				else if(mSpeedValue == 4)
					mAudioSink->setParameters(String8("time_stretch=4"));				
			}
		}
	}
#endif	
...
//#endif
    ...
    if (mAudioPlayer != NULL && !(mFlags & (AUDIO_RUNNING | SEEK_PREVIEW))) {
#if 0 //CR#ALPS01474624: skip early resume to avoid http streaming cache miss
		...
		if(SpeedValueChanged){
			ALOGD("startAudioPlayer_l,SpeedValueChanged ");
			if(mAudioSink != NULL){
				if(mSpeedValue == 1)
					mAudioSink->setParameters(String8("time_stretch=1"));
				else if(mSpeedValue == 2)
    ...
    
    {
		...
		if(mAudioSink == NULL){
			if(mSpeedValue==2)
				...
				mVideoTimeUs = mSync_time + (mVideoTimeUs-mSync_time)/4;
		}
#endif		
        if (mVideoTimeUs < 0) {
    ...
    if (wasSeeking == SEEK_VIDEO_ONLY) {
		...
		if(mAudioSink == NULL){
			if(mSpeedValue==2)
				...
				nowUs = mSync_time + (nowUs-mSync_time)/4;
...
#ifdef MTB_SUPPORT    
    ATRACE_BEGIN_EXT("release VB");
#endif    
#endif
    mVideoBuffer->release();
#ifndef ANDROID_DEFAULT_CODE
#ifdef MTB_SUPPORT
    ATRACE_END_EXT("release VB");
#endif    
