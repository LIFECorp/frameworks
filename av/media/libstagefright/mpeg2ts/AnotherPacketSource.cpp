...
#ifndef ANDROID_DEFAULT_CODE
void AnotherPacketSource::setPendingFormat(const sp<MetaData> &meta) {
    CHECK(mPendingFormat == NULL);

    if (meta == NULL) {
        return;
    }
    mPendingFormat = meta;
    ALOGD("Anotherpacketsource setpendingformat");
}

sp<MetaData> AnotherPacketSource::getPendingFormat() {
    return mPendingFormat;
}
#endif
...
#ifndef ANDROID_DEFAULT_CODE
            if(discontinuity & ATSParser::DISCONTINUITY_HTTPLIVE_BANDWIDTH_SWITCHING) {
                mFormat.clear();
                setFormat(mPendingFormat);
                ALOGD("set format via pendingFormat at anotherpacketsource");
...
#ifndef ANDROID_DEFAULT_CODE
void AnotherPacketSource::clear(bool bKeepFormat) {
    Mutex::Autolock autoLock(mLock);
    if (!mBuffers.empty()) {
    mBuffers.clear();
	}
...
void AnotherPacketSource::queueDiscontinuity(
    ...
    if ((mFormat != NULL) && wasFormatChange(type)) {
      int32_t width = 0, height = 0;
      mFormat->findInt32(kKeyWidth, &width);
      mFormat->findInt32(kKeyHeight, &height);
      ALOGD("AnotherPacketSource mFormat clear %d x %d", width , height);
      mFormat.clear();
    }
    */
