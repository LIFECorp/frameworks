...
//24bit->32bit sample
    ...
    if (mFlags & kUsePostProcessingFw) {
        ALOGD("setParameter OMX_IndexVendorMtkOmxVdecUsePostProcessingFw = %#x", OMX_IndexVendorMtkOmxVdecUsePostProcessingFw);
        mOMX->setParameter(mNode, OMX_IndexVendorMtkOmxVdecUsePostProcessingFw, &mPropFlags, sizeof(void*));
    }
#endif
...
status_t OMXCodec::setVideoOutputFormat(
    ...
    //} else if (!strcasecmp(MEDIA_MIMETYPE_VIDEO_VP9, mime)) {//?? need modify
...
status_t OMXCodec::freeBuffersOnPort(
    ...
    for (size_t i = buffers->size(); i-- > 0;) {
        ...
        if (onlyThoseWeOwn && info->mStatus == OWNED_BY_COMPONENT) {
			...
			SXLOGD("i %d, info->mStatus %d", i, info->mStatus);
            continue;
        }
		...
		SXLOGD("freeing buffer %p on port %ld, buffers->size() %d, i %d", info->mBuffer, portIndex, buffers->size(), i);
