...
#include "venc_drv_if_public.h" // for normal priority setting, chip driver dependent
...
#define TRACK_SKIPNOTIFY_ENABLE 1
...
#if TRACK_SKIPNOTIFY_ENABLE
    if( 0 == (mNotifyCounter%TRACK_SKIPNOTIFY_RATIO) )
    {
        ALOGV("notify nTotalBytesEstimate %lld, %d", nTotalBytesEstimate, mNotifyCounter);
...
#ifndef ANDROID_DEFAULT_CODE //for support multi-Slice case
#ifdef MTK_VIDEO_HEVC_SUPPORT
	    off64_t offset = ((chunk->mTrack->isAvc()|| chunk->mTrack->isHevc())&& !(chunk->mTrack->getMultiSliceBSInfo()))
#else
...
#ifndef ANDROID_DEFAULT_CODE
    VENC_DRV_MRESULT_T ret = eVEncDrvQueryCapability(VENC_DRV_QUERY_TYPE_NORMAL_PRIO, NULL, NULL);
    if(VENC_DRV_MRESULT_OK == ret)
    {
        // Track, Writer, and Encoder threads running at normal priority. Depends chip driver setting
...
status_t MPEG4Writer::Track::threadEntry() {
	...
	if(mIsAudio)
        ...
        if (buffer->meta_data()->findInt32(kKeyIsCodecConfig, &isCodecConfig)
            ...
            if (mIsAvc) {
             ...
             ALOGD("%s MPEG4 codec info size=%d", mIsAudio?"Audio":"Video", mCodecSpecificDataSize);
				...
				if(buffer->meta_data()->findInt32(KKeyMultiSliceBS,&iMultiSliceBS)
					&& iMultiSliceBS){
					ALOGI("HEVC multi Slice Bitstream %d",iMultiSliceBS);
					mMultiSliceBS = iMultiSliceBS;		
				}
				status_t err = makeHEVCCodecSpecificData(
						(const uint8_t *)buffer->data()
							+ buffer->range_offset(),
						buffer->range_length());
				ALOGD("HEVC codec info size=%d", buffer->range_length());
				CHECK_EQ((status_t)OK, err);
			}
#endif	
		...
		sampleSize = 0;
#ifdef MTK_VIDEO_HEVC_SUPPORT
		if ((mIsAvc || mIsHevc)&& (mStszTableEntries->count() == 0) && (mAVCSEIData != NULL))
		...
		if ((mIsAvc||mIsHevc) && !mMultiSliceBS) StripStartcode(copy);
		
		sampleSize = copy->range_length();
		
		if ((mIsAvc||mIsHevc) && !mMultiSliceBS) {
#else
