...
void ChromiumHTTPDataSource::onConnectionEstablished(
    ...
    if (mState != CONNECTING) {
	    ...
	    LOG_PRI(ANDROID_LOG_DEBUG, LOG_TAG, "onConnectEstablished, mState = %d, skip this connection and return", (int)mState);
	    //mCondition.broadcast();
            return;
        }	
#endif
...
void ChromiumHTTPDataSource::disconnect_l() {
    ...
    if (mState == CONNECT_REDIRECT) {
        LOG_PRI(ANDROID_LOG_DEBUG, LOG_TAG, "disconnect when CONNECT_REDIRECT!!!");
    }
#endif
