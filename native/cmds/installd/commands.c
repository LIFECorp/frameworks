...
int create_ext_cache_path(char path[PKG_PATH_MAX], const char *src)
{
    ...
    if (access(EXT_DALVIK_CACHE_PREFIX, R_OK) < 0) {
        if (mkdir(EXT_DALVIK_CACHE_PREFIX, 0771) < 0) {
            ALOGE("prepare_ext cache dir %s fail: %s", EXT_DALVIK_CACHE_PREFIX, strerror(errno));
            //return -1;
        }
        #if 0
        if (chown(EXT_DALVIK_CACHE_PREFIX, AID_SYSTEM, AID_SYSTEM) < 0) {
            ALOGE("chown %s fail: %s", EXT_DALVIK_CACHE_PREFIX, strerror(errno));
            //return -1;
        }

        if (chmod(EXT_DALVIK_CACHE_PREFIX, 0771) < 0) {
            ALOGE("chmod %s fail: %s", EXT_DALVIK_CACHE_PREFIX, strerror(errno));
            //return -1;
        }
        #endif
    }
    ...
    dstlen = srclen + strlen(EXT_DALVIK_CACHE_PREFIX) +
        strlen(DALVIK_CACHE_POSTFIX) + 1;
    ...
    sprintf(path,"%s%s%s",
            EXT_DALVIK_CACHE_PREFIX,
            src + 1, /* skip the leading / */
            DALVIK_CACHE_POSTFIX);

    for(tmp = path + strlen(EXT_DALVIK_CACHE_PREFIX); *tmp; tmp++) {
        if (*tmp == '/') {
            *tmp = '@';
        }
    }
...
#ifdef SUPPORT_ODEX_ON_SDCARD
int move_odex(const char *src, uid_t uid, int is_public)
{
    char src_odex[PKG_PATH_MAX];
    char dst_odex[PKG_PATH_MAX];

    ALOGD("move_odex %s", src);

    if (validate_apk_path(src)) return -1;

    if(strstr(src, "\/data\/app")){
        ALOGI("don't move_odex installed on /data/app");
        return -1;
    }

    if (create_cache_path(src_odex, src)) return -1;
    if (create_ext_cache_path(dst_odex, src)) return -1;
    ...
    if (lstat(src_odex, &st) < 0) {
        if (errno != ENOENT) {
            ALOGE("couldn't stat %s : %s\n", src_odex, strerror(errno));
            return -1;
        }
    } else {
        if (S_ISLNK(st.st_mode)) {
            ALOGE("No need to move link_to_odex : %s\n", src_odex);
            return 0;
        }
    }           
    ...
    fi = open(src_odex, O_RDONLY, 0);
    fo = open(dst_odex, O_RDWR | O_CREAT | O_EXCL, 0644);

    if (fi < 0)  {
        ALOGE("move_dex: cannot open '%s'(%s) for input\n", src_odex, strerror(errno));
        return  -1;
    }

    if (fo < 0)  {
        ALOGE("move_dex: cannot open '%s'(%s) for output\n", dst_odex, strerror(errno));
        close(fi);
        return  -1;
    }
    ...
    if (fi & fo)  {
        ...
        do  {
            ...
            if (rlen != wlen)  {
                ALOGE("Couldn't move %s -> %s : %s\n", src_odex, dst_odex, strerror(errno));
                close(fi);
                close(fo);
                return -1;
            }
    ...
    ALOGD("unlink %s\n", src_odex);
    if (unlink(src_odex) < 0) {
        ALOGE("Couldn't unlink %s: %s\n", src_odex, strerror(errno));
        return -1;
    }

    ALOGD("symlink %s\n", src_odex);
    if (symlink(dst_odex, src_odex) < 0) {
        ALOGE("couldn't symlink '%s' -> '%s': %s\n", src_odex, dst_odex, strerror(errno));
        return -1;
    }
