...
#ifndef MTK_DEFAULT_AOSP
    if (hwc.hasSecureComposition(mHwcDisplayId)) {
        if (haveGles && haveHwc) {
            compositionType = DisplaySurface::COMPOSITION_MIXED_SEC;
        } else if (haveGles) {
            ALOGW("Should not use secure composition with GLES mode !!");
            compositionType = DisplaySurface::COMPOSITION_GLES;
        } else if (haveHwc) {
            compositionType = DisplaySurface::COMPOSITION_HWC_SEC;
        } else {
            // TODO
            compositionType = DisplaySurface::COMPOSITION_HWC_SEC;
        }
...
void DisplayDevice::swapBuffers(HWComposer& hwc) const {
    ...
    if (hwc.initCheck() != NO_ERROR ||
        ...
        if (mType >= DISPLAY_VIRTUAL || mLayersSwapRequired) {
